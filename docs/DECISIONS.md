...
## ADR-154: Production Runtime Validation & Legacy Cleanup (Sprint 37.4)

**Status:** Accepted
**Date:** 2026-08-12
**Context:** Sprints 37.1–37.3 completed navigation reachability, visual polish, and functional UX completeness for the ten production screens, but left two open items: six legacy screens flagged by Sprint 37.1 as "unreachable, zero production references" were documented but not deleted, and no sprint had yet performed a full, single-pass, end-to-end re-verification of the entire runtime lifecycle (Auth → Runtime Session → Hydration → Dashboard Restore → Observer → Persistence → Feature UI → Logout → different-athlete login → restart) after three consecutive UI-focused sprints. This sprint's explicit mandate was final validation before Backend/Admin/AI infrastructure work begins — verify first, then remove only what the verification proves is genuinely dead, with no runtime/persistence/SQLite/visual redesign.

**Decision:**

**1. Route audit (read-only):** Re-walked the complete Expo Router tree (`app/app/**`). Every authenticated tab, secondary route (`goals`, `recovery`, `notifications`), settings sub-route, and `workout/{detail,history,analytics,records,session,summary,complete}` route resolves to a real screen component. `workout/history` → `workout/detail` and `workout/analytics` are reachable via `WorkoutRuntimeScreen`'s header buttons through the existing `navigation/isReachableRoute.ts` allowlist. `workout/records`, `workout/session`, and `workout/summary` remain route files with **no in-app navigation trigger** — the pre-Workout-Runtime-Experience handoff functions that would populate them (`setPendingExecutableSession`, the summary/records entry points) are called by nothing in production; only `workout/complete` is reachable, and only from within the already-unreachable `workout/session` flow. These three are documented here as **intentionally-inert legacy route files** (not deleted — Expo Router route files carry low ongoing cost, and their backing screens/hooks/repositories are exercised directly by their own dedicated tests) rather than "dead code" requiring removal; no navigation defect exists because nothing currently points at them.

**2. Legacy screen cleanup (executed):** Re-confirmed all six screens Sprint 37.1 flagged (`ProgressScreen.tsx`, `WorkoutScreen.tsx`, `DashboardScreen.tsx`, `NutritionScreen.tsx`, `CoachScreen.tsx`, `ProfileScreen.tsx`) still have **zero** production references — every real route imports its `*-experience`/`home`/`workout-runtime` replacement directly, never the legacy alias. Deleted all six, plus their two dedicated test files (`ProfileScreen.test.tsx`, `CoachScreen.test.tsx` — the only test files that exclusively exercised a deleted screen). Their transitive dependencies (`features/progress` UI hook, `features/nutrition` UI components, `features/coach`, `features/conversation`'s `useCoachConversation` hook, `features/prompt-builder`'s `useCoachPrompt` hook, `features/profile`'s edit-form stack, `features/shared`'s `useCurrentUser`) were audited and found to now be unreachable from any production path — but were **deliberately left in place**: each still carries its own independent test suite exercising the hook/component/service directly (not through the deleted screen), several sibling exports from the same folders remain live in production (`features/progress/providers/MockProgressService` behind `progress-experience`'s Mock provider; `features/nutrition`'s domain models behind `integrations/nutrition-progress`; `features/profile`'s `SettingsRow` behind `SettingsScreen`/`AppearanceScreen`/`ThemeScreen`), and this sprint's mandate scoped cleanup to the six named screens plus their own imports/exports — not a transitive dead-code sweep of entire feature folders, which risks removing legitimate test coverage for a future re-wiring. A follow-up cleanup sprint can retire the now-fully-orphaned leaf hooks (`useCoachChat`, `useCoachConversation`, `useCoachPrompt`'s legacy path, `useCurrentUser`, `useProfileEdit`) once their own dedicated tests are also confirmed removable.

**3. Mock/demo audit (read-only, zero defects found):** Searched the full authenticated route tree for `Mock*`, demo/sample/fake/placeholder strings, and hardcoded production values. Zero Category-A (production-reachable) findings — every production screen mounts its hook/ViewModel with no `service` override and takes the hydrated-runtime path; `Mock*ExperienceService` factories are reached only when a test or preview explicitly injects a `service` prop. `MockProgressAnalyticsService` (the documented sole working Progress Analytics implementation, Sprint 35.3/37.1) and the Sprint 30.x adapter family (`MockAuthenticationProvider`, `MockBackendProvider`, `MockLogger`) are confirmed intentional production infrastructure, unchanged.

**4–9. Runtime lifecycle, athlete isolation, persistence/restart, failure recovery, and write-through concurrency (read-only, zero defects found):** Re-verified against current code, with file/line evidence, that: the authenticated startup order (Bootstrap → Hydration → Dashboard Restore → Observer → ready) is unchanged since Sprint 33.6; `retrySession()`'s unconditional full pipeline reset (Sprint 36.3) and the logout reset cascade (Observer → Write-Through → Dashboard Restore → Hydration → Bootstrap-incl.-Composition-Root → Session, Sprint 33.8/36.1) are both intact; hydration's athlete-scope filter (Sprint 36.1) and payload-internal ownership guard (Sprint 36.5) remain wired into every domain's `observe*`/`restore*` function; all eight persisted domains (Identity, Workspace, Workout, Nutrition, Recovery, Goals, Coach, Notifications) round-trip through the same SQLite-backed repository contracts; `triggerWriteThrough()`'s `.catch()` (Sprint 36.4) still prevents unhandled rejections and `RuntimeFailureScreen`/`ErrorBoundary` (Sprint 36.2) still show no raw exceptions; and the Sprint 36.5/36.6 write-through concurrency guards — pre-I/O stale-sequence rejection, the post-I/O second sequence re-check, the never-per-mutation-reset `sessionEpoch` counter, `stale_write_through_epoch`, and `RuntimeObserver.start()`'s incremental partial-wrap cleanup — are all present and unmodified. No code changes were made to any runtime/persistence file; this sprint's mandate was verification, not rewrite, and none of the nine areas showed a concrete, provable defect.

**10. Performance audit (read-only, zero defects found):** Catalogued every `setInterval`/`setTimeout`/`requestAnimationFrame` in `app/src` (excluding tests). Confirmed the Sprint 36.6 rest-timer fix remains intact — `useRestTimer`'s two 1-second intervals call `WorkoutRuntimeViewModel.tickRestTimer()`/`tickDuration()`, which call only `notifyListeners()` (no write-through), while the separate `notify()` path used by real mutations still triggers persistence exactly as designed. No polling exists in `progress-experience`, `progress-analytics`, `coach-experience`, or `notification-center`; Coach's typing indicator uses a Reanimated `withRepeat` loop, not a JS timer. Every interval/timeout found has matching cleanup; none touch SQLite or persistence from a render body.

**11. Accessibility/UX regression audit (three narrow fixes):** Sprint 37.2/37.3 changes were checked component-by-component. Found and fixed three concrete regressions, each the smallest possible correction:
   - `TimeRangeSelector` used `AppButton`'s primary/secondary variant to show the active time range visually, but `AppButton` only exposed `disabled`/`busy` accessibility state — a screen reader had no way to tell which range was currently selected. `AppButton` gained an optional `selected?: boolean` prop (additive, `undefined` by default, zero behavior change for its ~20 other call sites) that sets `accessibilityState.selected`; `TimeRangeSelector` now passes `selected={option.value === value}`.
   - `NotificationCard`'s `accessibilityLabel` covered only title and read/unread state, silently dropping the high-priority `Chip` that Sprint 37.2 added visually — a screen reader user had no way to know a notification was flagged urgent/high. The label now includes `"{priority} priority."` when applicable.
   - `GoalsCard` and `NotificationPreferencesCard` became pressable link-outs in Sprint 37.3 (navigating to Goals/Notifications respectively) but relied on `AppCard`'s implicit child-text accessible name instead of an explicit label describing the navigation — both now pass an explicit `accessibilityLabel` describing the destination and purpose.
   No visual change in any of the three fixes — all are accessibility-metadata-only.

**12. Tests/typecheck:** `npm test -- --no-coverage` and `npm run typecheck` both pass. Suite count dropped from 812/3369 to 810/3354 (2 suites, 15 tests) — exactly the two obsolete legacy-screen test files removed in Part 2; no other test was touched or lost.

**Alternatives considered:**
- **Delete `workout/records`/`workout/session`/`workout/summary` route files since nothing navigates to them** — rejected; unlike the six named legacy screens, these were never flagged by name in this sprint's brief, their backing screens (`WorkoutRecordsScreen`, `WorkoutSessionScreen`, `WorkoutSummaryScreen`) have their own passing test suites exercising real behavior (not stubs), and the brief explicitly warns "do not delete anything merely because it looks old" — documenting them as intentionally-inert is the provably-safe choice; deleting live route files with tested backing screens on the strength of an audit alone risks removing a future re-entry point (e.g., a "workout records" quick action) that a later sprint may deliberately wire up.
- **Cascade-delete the now-fully-orphaned `features/coach`, `features/conversation`'s `useCoachConversation`, `features/prompt-builder`'s `useCoachPrompt`, `features/profile`'s edit stack, and `features/shared`'s `useCurrentUser`** — deferred; each still has its own independent test suite that would need to be evaluated and removed test-by-test (not screen-by-screen), which is a materially larger, separately-scoped cleanup than "verify six named screens are still dead and remove them"; doing it inline here risked exceeding this sprint's proven-safe boundary.
- **Fix `NotificationCard`'s inert action `<Text>` labels (no `onPress`/role) found during the accessibility audit** — deferred; this is a feature-completeness gap (no handler exists to wire), not an accessibility *regression* introduced by 37.2/37.3 visual changes — the actions were never interactive before the redesign either; in scope for a future notification-actions sprint, not this validation pass.

**Consequences:**
- Documentation: [PROJECT_STATE.md](./PROJECT_STATE.md), [CHANGELOG.md](./CHANGELOG.md).
- Six legacy screens and two orphaned test files removed with proof of zero production references; every other audited area (routes, mocks, runtime lifecycle, athlete isolation, persistence/restart, failure recovery, write-through concurrency, performance) verified correct with no code change; three narrow, additive accessibility fixes closed real 37.2/37.3 regressions with no visual redesign. No runtime architecture change, no new runtime subsystem, no networking, no backend, no SQLite/schema change, no Admin, no live LLM, no repository contract change. Full suite (810 suites / 3354 tests) and typecheck remain green. The application is now considered validated and ready for the next phase (Backend / Admin / AI infrastructure).

---

## ADR-155: Profile Mobile ↔ Backend Integration (Phase A.1)

**Status:** Accepted
**Date:** 2026-08-12
**Context:** ADR-154 confirmed every production screen — including `ProfileExperienceScreen` — runs entirely on the runtime-hydrated path (`AthleteIdentityService` → SQLite via Runtime Observer) with `ProfileExperienceService`'s `backend` provider left as a documented placeholder that only threw ("Backend profile experience provider is not configured."). Phase A.1 kicks off Backend infrastructure work by connecting Profile to the already-implemented `GET`/`PATCH /api/v1/users/me` (see `backend/app/api/v1/users.py`, `UserService`, `UserRepository`) without redesigning Runtime Session, SQLite, Runtime Observer, or introducing a new persistence layer — matching the reference pattern already proven for Workout (`BackendWorkoutService`, ADR-… Sprint 6.3) and User (`BackendUserService`, Sprint 6.0–6.1).

Auditing `UserPublic`/`UserUpdate` (`backend/app/schemas/user.py`) against the Profile Experience read model (`AthleteProfile`) showed the backend owns exactly: name, email, birth date, height, weight, activity level, goal, and account status. It has no field for training/nutrition/coach/notification/appearance preferences, measurement-unit choice, or goal-progress entries — those remain correctly local-only/unsupported, as ADR-153 already established for Goals/Notifications. Of the backend-owned fields, `heightCm`/`weightKg` were the one clear, previously-unconnected gap: `mapAthleteIdentityToProfile` hardcodes both to `null` because Athlete Identity has no such field at all — there was no local value to preserve, and no existing Application API to update them.

**Decision:**

1. **`BackendProfileExperienceService` implemented for real** (`getProfile`/`updateAthleteInfo`), reusing the existing authenticated API client (`src/api/client.ts`'s `request()`, via the existing `src/api/auth.ts#getCurrentUser` and `src/api/users.ts#updateCurrentUser` — no new endpoints, schemas, or networking infrastructure). A new mapper, `mapUserPublicToProfileDto`, projects `UserPublic` into `AthleteProfileDto`, populating only the backend-owned fields and reusing the exact same "unsupported domain" defaults `mapAthleteIdentityToProfile` already uses (extracted as `DEFAULT_SECTIONS`/`DEFAULT_CONNECTED_SERVICES`) rather than inventing backend data. `toNumberOrNull`/`buildDisplayName` (the `Decimal`-as-string coercion and name-composition helpers already proven in `features/shared/utils/userAdapters.ts`) are reused, not duplicated. The provider's other seven update methods (training/nutrition/coach/notification/appearance/units/goals) now throw an explicit `"<X> is not supported by the backend user API yet"` `ProfileExperienceError` instead of a generic "not configured" stub message — same fail-clearly behavior as ADR-153, worded for what is actually true today.
2. **New `ProfileExperienceService.updateAthleteInfo(input: AthleteInfoUpdateDto)`** contract method (implemented by all three providers — Mock merges in-memory, Local keeps the existing "not configured" placeholder, Backend PATCHes for real) plus a matching Application API (`application/UpdateAthleteInfo.ts`), following the exact `if (deps.service) {…} / else backend-through-runtime` shape every other Update* Application function already uses, and a `ProfileExperienceViewModel.updateAthleteInfo()` method/`useProfile().updateAthleteInfo` hook binding alongside the existing update methods.
3. **Runtime-driven production path calls the backend directly for `heightCm`/`weightKg`.** Since Athlete Identity has no field for them, there is no local write to perform and no SQLite/Runtime Observer change is needed or made — `UpdateAthleteInfo`'s runtime branch calls `updateCurrentUser()` directly (the same authenticated client used everywhere else) and merges the response's `height_cm`/`current_weight_kg` into the currently-hydrated `AthleteProfile` in memory. On failure, the error propagates as a `ProfileExperienceError` (never the raw `ApiError`/`Error`) and the previously-hydrated profile is left untouched — no "successful save" is ever reported for a write that didn't happen, matching the ADR-153 fail-clearly precedent.
4. **No UI changes.** `AthleteCard`/`ProfileExperienceScreen` are unchanged — this phase wires the Application/ViewModel/Provider seam per the brief; a future sprint can add the actual edit affordance.

**Alternatives considered:**
- **Force-map `trainingPreferences.level`/`goal`-kind AthleteGoal entries onto the backend's `activity_level`/`goal` enums** — rejected; these are different domains with different value sets (workout-experience level and progress-tracked goals vs. a coarse self-reported activity/weight-goal enum); mapping them would invent a backend contract that doesn't exist, violating the brief's explicit "do not invent backend fields" constraint.
- **Fetch backend `height_cm`/`current_weight_kg`/`birth_date` during the existing synchronous `readHydratedProfile()` read path** — rejected; that function is called synchronously from `useProfile`'s effect against the Runtime Session/Athlete Identity read path — turning it async to add a network call would be a Runtime Session shape change the brief explicitly prohibits. The write path (`updateAthleteInfo`) already keeps the in-memory profile current after every successful save, which is sufficient for this phase.
- **Route the runtime-path write through `AthleteIdentityService.build()` like every other Update* function** — rejected; Athlete Identity has no `heightCm`/`weightKg` field to build, so there's nothing to write there — doing so would mean either extending the Athlete Identity domain model (a persistence-layer/SQLite schema change the brief explicitly prohibits) or silently discarding the value, the exact "pretend success" failure mode ADR-153 closed.

**Consequences:**
- Documentation: [PROFILE_EXPERIENCE_ARCHITECTURE.md](./PROFILE_EXPERIENCE_ARCHITECTURE.md), [FRONTEND_STATUS.md](./FRONTEND_STATUS.md), [PROJECT_STATE.md](./PROJECT_STATE.md), [CHANGELOG.md](./CHANGELOG.md).
