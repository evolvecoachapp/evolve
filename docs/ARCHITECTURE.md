...
### Runtime Persistence Failure Recovery (`runtime/runtime-observer`, `runtime/write-through`) — Sprint 36.4

| Aspect | Implementation |
|--------|----------------|
| **Purpose** | Guarantee that a failed Runtime Write-Through (repository) call cannot leave the application in an inconsistent persistence state — no unhandled promise rejection, no rolled-back domain mutation, no session/observer failure, and deterministic recovery on the next successful mutation |
| **Flow — failure** | Runtime mutation succeeds → `RuntimeObserver` detects `build()` success → `triggerWriteThrough()` → `resetRuntimeWriteThrough()` → `persistRuntime()` → `RuntimeWriteThroughPipeline.persist()` → repository `save()` throws → write-through status = `"failed"` (existing `RuntimeWriteThroughState.error`) → rejection is caught inside `RuntimeObserver`'s `triggerWriteThrough()` and logged via `getLogger().error()` (`infrastructure/logging`) — never surfaced as an unhandled promise rejection, never rethrown into the `build()` call site |
| **Runtime state is never rolled back** | The domain mutation that triggered the failed persist already completed successfully in memory before write-through ran; `RuntimeWriteThroughPipeline.persist()`'s catch block only records `status: "failed"` + `error` — it never touches any runtime composition service. Requesting the just-mutated state from the service (e.g. `AthleteIdentityService.getAthleteIdentity()`, `WorkoutRuntimePersistenceService.getState()`) always returns the latest in-memory value regardless of the persistence outcome |
| **Observer stays installed; Session stays READY** | A write-through failure only ever changes `RuntimeWriteThroughState.status`. `RuntimeObserver`'s own status remains `"ready"` (no observer restart, no re-wrap) and `RuntimeSessionContext`'s `status` remains `"ready"` — persistence failure and runtime/session availability are separate concerns by construction; nothing in the failure path calls `resetRuntimeObserver()`, `resetRuntimeSession()`, or any session-status setter |
| **Recovery on the next mutation** | `triggerWriteThrough()` (introduced with the observer in Sprint 33.7/35.4) already calls `resetRuntimeWriteThrough()` **before every** `persistRuntime()` call — not only after a detected failure. This is what makes recovery deterministic: `persistRuntime()` treats an already-`"ready"` state as idempotent (returns the cached result) and would otherwise keep returning the same stale **rejected** promise forever after a `"failed"` state, since only `resetRuntimeWriteThrough()` clears the in-flight promise reference. Resetting unconditionally on every successful `build()` guarantees the very next mutation — success, failure, or first-ever — always re-observes and persists the complete current runtime state through the existing reset + persist APIs, with no new retry/queue/debounce mechanism |
| **Latest mutation wins** | Write-through persists full current snapshots read from the runtime services at execution time (not deltas captured at trigger time), so any later successful persist cycle always writes whatever is currently in memory — a prior failed or partial cycle can never overwrite a later successful one. A + B(fails) + C(succeeds) always leaves SQLite (and post-restart hydration) reflecting C |
| **No duplicate wrappers** | The fix is scoped to `triggerWriteThrough()`'s fire-and-forget persist call (`.catch()` added around `void persist(...)`); `RuntimeObserver.start()`'s `build()` wrapping/unwrapping (Sprint 33.7 / 36.3) is untouched — one wrap per service, one persist trigger per successful `build()`, whether or not the previous attempt failed |
| **Domain coverage** | Workout / Nutrition / Recovery persist through their own `*RuntimePersistenceService.build()` (own repositories); Goal Progress / Coach / Notification persist through `UnifiedWorkspaceService.build()` → `WorkspaceRepository` (existing overlays, Sprint 35.4/35.5). All six route through the same `RuntimeObserver` → `triggerWriteThrough()` → `persistRuntime()` path, so the same failure/recovery semantics apply uniformly without any domain-specific handling |
| **Logout unaffected** | Logout's existing reset cascade (Sprint 33.8/36.1/36.3: `resetRuntimeObserver()` → `resetRuntimeWriteThrough()` → `resetDashboardRestore()` → `resetRepositoryHydration()` → `resetRuntimeBootstrap()` → `resetRuntimeSession()`) already unconditionally clears write-through status/error/in-flight-promise regardless of whether the last attempt failed — no code path change required; a persistence failure never blocks or alters logout |
| **Athlete isolation (unchanged)** | Recovery reuses the same authenticated `athleteIds` already threaded through `RuntimeObserver`/`persistRuntime()` (ADR-125/145/147) — no cached/stale athlete id is ever introduced by the failure/recovery path |
| **Composition Root** | None — no new services, tokens, or factories; reuses `RuntimeWriteThroughService`/`RuntimeObserverService` (tokens #61/#63) exactly as registered |
| **Design** | **Hardening only.** No new persistence architecture, no retry queue, no background worker, no debounce/batching, no event bus, no new repository/provider, no SQLite changes, no networking, no Admin, no AI/Coach changes, no visual redesign |

Decision record: ADR-148 in [DECISIONS.md](./DECISIONS.md).

### Runtime Persistence Verification & Consistency Guard (`runtime/persistence`, `runtime/write-through/RuntimeWriteThroughSequence`) — Sprint 36.5

| Aspect | Implementation |
|--------|----------------|
| **Purpose** | Final consistency hardening pass over the complete persistence lifecycle (Runtime Session → Observer → Domain Mutation → Write-Through → Repository Contracts → SQLite → Hydration → Restore). A verification sprint: confirms the chain established by Sprints 35.4/35.5/36.1/36.4 cannot silently produce stale or cross-athlete runtime state, and adds the two smallest possible guards needed to close the concrete gaps found |
| **Gap 1 — payload-internal athlete mismatch** | The Sprint 36.1 Authenticated Athlete Persistence Boundary (`filterRecordsForAthleteScope()`) validates a record's own key (`record.id`) against the session's `athleteIds`, but never checked whether a payload's *internal* `athleteId` field actually agreed with that key. A corrupted or mis-keyed row (record legitimately keyed under the current athlete, payload internally tagged for a different one) could otherwise be persisted or restored under the wrong identity |
| **Guard 1 — `AthleteRecordOwnership`** | New pure module `runtime/persistence/AthleteRecordOwnership.ts` — `isOwnedByAthlete(payload, expectedAthleteId)` and `isRecordOwnedByAthlete(record, payload)`, both simple equality checks, no state. Wired into every `observe*Records()` function in `RuntimeWriteThroughPersistence.ts` (identity, workspace, snapshot, timeline, workout, nutrition, recovery) — a payload whose `athleteId` disagrees with the athlete id being persisted is skipped and logged (`getLogger().warn()`), never written. Wired into every `restore*Records()` function in `HydrationRestoration.ts` (identity, workspace, workout, nutrition, recovery, timeline, coach overlay) — a payload whose `athleteId` disagrees with its own record key is skipped and logged, never restored into a composition service. `restoreSnapshotRecords()` is intentionally excluded and commented: `AthleteSnapshot` records are keyed by a composite id (`athlete-snapshot:${athleteId}:${createdAt}`), not `athleteId` directly, so the `record.id === payload.athleteId` check does not apply to that keying convention — a pre-existing, out-of-scope structural note, not a regression |
| **Gap 2 — out-of-order write-through completion** | `triggerWriteThrough()` (Observer, Sprint 33.7/36.4) resets Write-Through state immediately before every persist call, including while a previous persist call may still be executing asynchronously. If a later mutation's persist call completes *before* an earlier mutation's still-in-flight persist call, the earlier call's write could land last and silently downgrade the repository/state-holder back to older data, even though every persist call reads live current state |
| **Guard 2 — monotonic mutation sequence** | New pure counter module `runtime/write-through/RuntimeWriteThroughSequence.ts` — `nextRuntimeMutationSequence()` increments and returns a sequence number, called once per successful mutation inside `RuntimeObserver`'s `onSuccessfulChange()`; the number is threaded through `persistRuntime({ mutationSequence })` → `RuntimeWriteThroughPipeline.persist({ mutationSequence })`. `persist()` rejects immediately — before observing any runtime service or touching any repository — with a new `RuntimeWriteThroughError` code `stale_mutation_sequence` if `mutationSequence` is older than the sequence already reflected by a completed persist (`isStaleWriteThroughSequence()`); `markWriteThroughSequenceApplied()` records the applied sequence only after a persist call actually succeeds. `mutationSequence` is optional everywhere — omitting it preserves the exact prior behavior for any other caller. `resetRuntimeMutationSequence()` is called from `RuntimeObserver.stop()`, joining the existing reset cascade — no new reset API surface |
| **No event sourcing / queue / retry** | Both guards are pure functions and module-level counters composed into the *existing* observe/restore/persist call sites — no versioned event log, no background worker, no queue, no retry system, no network synchronization, no new repository. `RuntimeWriteThroughOptions`/`PersistRuntimeOptions` gained one optional field each; every other public shape is unchanged |
| **Verification (no code change)** | Mutation sequencing (A → persist A, B → persist B, C → repository failure, D → persist D — SQLite/hydration reflect only D), failure sequencing (persist fails → runtime state remains valid → observer stays `"ready"` → next mutation recovers), and persistence-failure consistency were already proven correct by Sprint 36.4's tests and re-confirmed here without modification — no gap found, no guard added |
| **Athlete isolation (re-verified, extended)** | The A → logout → B → logout → A round trip (Sprint 36.1) was already covered for identity/workspace; Sprint 36.5 adds the same round trip across all cross-domain overlays (Workout/Nutrition/Recovery/Goal/Coach/Notification) in one integration test, confirming no domain-specific isolation gap exists |
| **Cross-domain / dashboard / reset verification (new tests, no code change)** | New integration coverage: a single runtime session mutating all eight domains (Identity/Workspace + Workout/Nutrition/Recovery/Goal/Coach/Notification) persists and restores together after a full restart; `restoreDashboard()` after hydration reflects the real hydrated workspace (`isEmpty: false`, real `displayName`) and never the empty/mock fallback shape; a comprehensive full-reset test asserts every pipeline's status **and** result (`RuntimeObserverStateHolder`, `RuntimeWriteThroughStateHolder`, `HydrationStateHolder`, `DashboardRestoreStateHolder`, `BootstrapStateHolder`) returns to idle/null, `RuntimeObserverStateHolder().athleteIds` returns to `[]`, and the mutation sequence counters (Guard 2) return to `0` — none of these were previously asserted together in one place |
| **Composition Root** | None — no new services, tokens, or factories; both guards are pure modules imported directly by existing pipeline code |
| **Design** | **Verification + guard only.** No runtime architecture redesign, no new runtime subsystem, no networking, no backend, no SQLite changes, no Admin changes, no AI/Coach changes, no visual redesign |

Decision record: ADR-149 in [DECISIONS.md](./DECISIONS.md).

### Production Runtime Validation & Legacy Cleanup (`navigation/isReachableRoute`, `components/AppButton`, `features/notification-center`, `features/profile-experience`, `features/progress-experience`) — Sprint 37.4 (Phase B — Product Completion)

| Aspect | Implementation |
|--------|----------------|
| **Purpose** | Final validation-and-cleanup sprint before Backend/Admin/AI infrastructure work begins. Re-audits the complete production lifecycle (Auth → Runtime Session → Hydration → Dashboard Restore → Observer → Persistence → Feature UI → Logout → different-athlete login → Restart) end-to-end, under an explicit "verify first, remove only what's proven dead, no redesign" mandate — the counterpart hardening pass to Sprint 36.6, but after three UI-focused sprints (37.1–37.3) rather than before them |
| **Route audit (read-only)** | Re-walked the full Expo Router tree; every authenticated route resolves to a real screen. `workout/history` → `workout/detail` and `workout/analytics` are reachable via `WorkoutRuntimeScreen`'s header buttons through the existing `isReachableRoute.ts` allowlist. `workout/records`, `workout/session`, and `workout/summary` remain route files with **no in-app navigation trigger** — the pre-Workout-Runtime-Experience handoff functions that would populate them (e.g. `setPendingExecutableSession`) are called by nothing in production; only `workout/complete` is reachable, and only from within the already-unreachable `workout/session` flow. Documented as intentionally-inert legacy route files, not deleted — never named in this sprint's cleanup list, and each backed by a screen with its own passing test suite exercising real (non-stub) behavior |
| **Legacy screen cleanup (executed)** | Re-confirmed all six screens Sprint 37.1 flagged (`ProgressScreen.tsx`, `WorkoutScreen.tsx`, `DashboardScreen.tsx`, `NutritionScreen.tsx`, `CoachScreen.tsx`, `ProfileScreen.tsx`) still have zero production references — every real route imports its `*-experience`/`home`/`workout-runtime` replacement directly, never the legacy alias. Deleted all six, plus `ProfileScreen.test.tsx`/`CoachScreen.test.tsx` (the only tests exclusively exercising a deleted screen). Their transitive dependencies (`features/progress`'s `useProgress` hook, `features/nutrition`'s legacy `NutritionHero`/`NutritionMacroGrid`/`NutritionMealCard`, `features/coach` incl. the already-deprecated `useCoachChat`, `features/conversation`'s `useCoachConversation` hook, `features/prompt-builder`'s `useCoachPrompt` hook, `features/profile`'s `ProfileEditForm`/`ProfileInfoReadCard`/`useProfileEdit`, `features/shared`'s `useCurrentUser`) are now themselves unreachable from production but were **not** deleted — each retains its own independent test suite exercising it directly (not through a deleted screen), and several sibling exports from the same folders remain live in production (`features/progress/providers/MockProgressService` behind `progress-experience`'s Mock provider; `features/nutrition`'s domain models behind `integrations/nutrition-progress`; `features/profile`'s `SettingsRow` behind `SettingsScreen`/`AppearanceScreen`/`ThemeScreen`). This sprint's mandate scoped cleanup to the six named screens and their own imports/exports, not a transitive dead-code sweep of entire feature folders — deferred to a future cleanup sprint |
| **Mock/demo audit (read-only, zero defects)** | Searched the full authenticated route tree for `Mock*`, demo/sample/fake/placeholder strings, hardcoded production values. Zero production-reachable findings — every production screen mounts its hook/ViewModel with no `service` override and takes the hydrated-runtime path; `Mock*ExperienceService` factories are reached only when a test/preview explicitly injects `service`. `MockProgressAnalyticsService` (Sprint 35.3/37.1) and the Sprint 30.x Mock adapter family (`MockAuthenticationProvider`/`MockBackendProvider`/`MockLogger`) reconfirmed as intentional production infrastructure |
| **Runtime lifecycle / athlete isolation / persistence-restart / failure-recovery / write-through concurrency (read-only, zero defects)** | Re-verified against current code, with file/line evidence: authenticated startup order (Bootstrap → Hydration → Dashboard Restore → Observer → ready, unchanged since Sprint 33.6); `retrySession()`'s unconditional full pipeline reset (Sprint 36.3); the logout reset cascade (Observer → Write-Through → Dashboard Restore → Hydration → Bootstrap-incl.-Composition-Root → Session, Sprint 33.8/36.1); hydration's athlete-scope filter (Sprint 36.1) and payload-internal ownership guard (Sprint 36.5) wired into every domain's `observe*`/`restore*` function; all eight persisted domains (Identity/Workspace/Workout/Nutrition/Recovery/Goals/Coach/Notifications) round-tripping through the same SQLite-backed repository contracts; `triggerWriteThrough()`'s `.catch()` (Sprint 36.4) preventing unhandled rejections; `RuntimeFailureScreen`/`ErrorBoundary` (Sprint 36.2) showing no raw exceptions; and the Sprint 36.5/36.6 write-through concurrency guards (pre-I/O stale-sequence rejection, post-I/O second sequence re-check, never-per-mutation-reset `sessionEpoch`, `stale_write_through_epoch`, `RuntimeObserver.start()`'s incremental partial-wrap cleanup) — all confirmed intact. No runtime/persistence code was changed; none of the nine areas showed a concrete, provable defect |
| **Performance audit (read-only, zero defects)** | Catalogued every `setInterval`/`setTimeout`/`requestAnimationFrame` in `app/src` (excluding tests). Confirmed the Sprint 36.6 rest-timer fix remains intact — `useRestTimer`'s two 1-second intervals call `WorkoutRuntimeViewModel.tickRestTimer()`/`tickDuration()`, which call only `notifyListeners()` (no write-through), while `notify()` (real mutations) still triggers persistence exactly as designed. No polling in `progress-experience`, `progress-analytics`, `coach-experience`, or `notification-center` (Coach's typing indicator uses a Reanimated `withRepeat` loop, not a JS timer). Every interval/timeout found has matching cleanup; none touch SQLite/persistence from a render body |
| **Accessibility/UX regression audit (three narrow fixes)** | `TimeRangeSelector` used `AppButton`'s primary/secondary variant to show the active range visually, but `AppButton` only exposed `disabled`/`busy` accessibility state, leaving the active range undetectable to a screen reader — `AppButton` gained an additive, opt-in `selected?: boolean` prop (`undefined` by default, zero behavior change for its ~20 other call sites) setting `accessibilityState.selected`; `TimeRangeSelector` now passes `selected={option.value === value}`. `NotificationCard`'s `accessibilityLabel` covered only title and read/unread state, silently dropping the Sprint 37.2 high-priority `Chip` — the label now includes `"{priority} priority."` when applicable. `GoalsCard`/`NotificationPreferencesCard` (made pressable link-outs in Sprint 37.3) relied on `AppCard`'s implicit child-text accessible name instead of an explicit label describing the navigation — both now pass an explicit `accessibilityLabel`. No visual change in any of the three fixes |
| **Tests** | `npm test -- --no-coverage` and `npm run typecheck` both pass; suite count 812 suites / 3369 tests → 810 suites / 3354 tests — exactly the two removed legacy-screen test files, no other test lost or modified |
| **Design** | **Verification + narrow cleanup + narrow accessibility fixes only.** No runtime architecture change, no new runtime subsystem, no networking, no backend, no SQLite/schema change, no Admin change, no live LLM provider, no repository contract change, no visual redesign |

Decision record: ADR-154 in [DECISIONS.md](./DECISIONS.md).

### Production UX Completion & Feature Surface Polish (`features/workout-runtime`, `features/nutrition-experience`, `features/recovery-experience`, `features/coach-experience`, `features/notification-center`, `features/progress-experience`, `features/profile-experience`) — Sprint 37.3 (Phase B — Product Completion)

| Aspect | Implementation |
|--------|----------------|
| **Purpose** | Functional-completeness audit across the ten production screens after Sprint 37.1 (reachability) and Sprint 37.2 (visual polish). Inspected primary/secondary actions, empty/loading/error states, save/update feedback, disabled states, and accessibility, then closed concrete gaps by wiring existing-but-unwired ViewModel/application capabilities — no new domain behavior, no runtime/persistence/SQLite architecture change |
| **Workout — completion final state** | `WorkoutRuntimeScreen` kept showing active set editors after `runtime.state.isCompleted`; new presentation-only `WorkoutCompleteCard` (checkmark, duration, sets-completed summary) replaces the editors, `WorkoutRuntimeHeader` gained a "Completed" `Chip` |
| **Nutrition — hydration logging** | `NutritionExperienceViewModel.logHydration()` existed and persisted correctly but had no UI; `HydrationCard` gained an `onLogHydration` prop and a Log button (250 mL increment, disabled once the goal is reached), wired in `NutritionExperienceScreen` |
| **Recovery — day-navigation bug fix** | `mapWorkspaceRecoveryToExperienceDto`'s `buildAvailableDays()` cloned the *same* `isoDate` onto every day option instead of shifting ±1 day (new `shiftIsoDate()` helper fixes it); `RecoveryExperienceViewModel.changeDay()` was re-passing the current day's in-memory sleep/readiness values as overrides into `loadHydratedRecoveryExperience()`, stomping the target day's own persisted state — now passes only `{athleteId, day}` and lets per-day persistence (Sprint 35.4) resolve on its own |
| **Coach — inline errors, honest status copy** | `CoachExperienceScreen` replaced the entire conversation with a full-page `CoachError` for *any* error, including transient send/regenerate failures; new compact, retryable `CoachInlineError` renders alongside the conversation for mutation errors, while `CoachError` is reserved for genuine load failures (`!conversation.experience`); `CoachStatus`'s "Streaming response…" (implying live token-by-token LLM streaming, which deterministic Coach does not perform, Sprint 35.1) corrected to "Coach is responding…" |
| **Notifications — mutation wiring + reminder creation** | `markRead`/`addReminder`/`editReminder`/`removeReminder`/`updateSettings` (Sprint 35.2) were never called from `NotificationCenterScreen`; `NotificationCard.onPress` now marks an unread notification read, `ReminderCard` gained `onToggleEnabled`/`onRemove`, `NotificationSettingsCard`'s boolean rows became interactive `Chip`s wired to `updateSettings` (disabled while saving). New `REMINDER_PRESETS` model + `AddReminderRow` component add the previously-missing reminder *create* entry point (Workout/Hydration/Sleep/Nutrition/Recovery quick-add presets calling the existing `addReminder()`) — no new persistence. Screen restructured so an empty inbox no longer hides the Reminders/Settings sections behind one all-or-nothing empty-state gate |
| **Progress — blank-screen fix on time-range switch** | `showContent` required `!loading.isLoading`, so every `changeTimeRange()` call (which never clears the existing `dashboard`) opened a gap where neither the skeleton (dashboard already existed) nor the content branch (loading was true) rendered — `showContent` now gates only on having a `dashboard` and no `error`, so the previous range's data stays visible until the new range's data replaces it; `TimeRangeSelector` gained a `disabled` prop to prevent overlapping switches |
| **Profile — unsupported fields now fail clearly (most significant fix)** | `updateGoals()`/`updateNotificationPreferences()`'s runtime-path branches called `readRuntimeProfile()`/returned unchanged data without ever applying the caller's input, because goals/notification preferences are owned by `goal-progress-experience`/`notification-center`, not Athlete Identity — no identity field existed to write into. `ProfileExperienceViewModel.save()` then reported this as a successful save (no thrown error) — the "silently pretend a setting was persisted" failure mode the sprint brief calls out by name. Both now throw a descriptive `ProfileExperienceError` instead |
| **Profile — display-only cards linked to their owning tab** | `GoalsCard`/`NotificationPreferencesCard` were static re-renderings of data with an already-correct, already-persisted home elsewhere; both now link out (`onPress` → `/(app)/goals` / `/(app)/notifications`, gated by `isReachableRoute`) instead of showing a possibly-stale duplicate. `NotificationPreferencesCard`'s on/off status dots in particular never reflected the real `NotificationSettings` at all — the DTO fields were hardcoded `false` in `mapAthleteIdentityToProfile` |
| **Profile — theme display now matches the real switch** | Athlete Identity's `settings.appearance` (what `AppearanceCard` displayed) is never written to by the actual theme switch (Settings → Appearance → Theme, backed by `ThemeContext` + `AsyncStorage`) — Profile could show a stale theme forever. `ProfileExperienceScreen` now passes the live `useTheme().preference` through to `AppearanceCard` for display, without touching either persistence path |
| **Profile — honest zero/unset values** | `TrainingPreferencesCard`/`NutritionPreferencesCard` displayed hardcoded `0` (sessions/week, duration, calories, meals/day — fields with no Athlete Identity backing) indistinguishable from real configured data; both now render `"—"` and hide empty tag rows, matching the unset-value convention already used by `AthleteCard`/`SleepCard`/`ReadinessCard` |
| **Accessibility** | `AppButton.accessibilityLabel` now falls back explicitly to `label` (previously relied on the `Text` child's implicit accessible name, absent during `loading`'s bare `ActivityIndicator`) and reports `accessibilityState.busy`; `Chip` gained a `disabled` prop keeping a disabled toggle visible-but-inert (`accessibilityState.disabled` + real RN `disabled`) instead of stripping `onPress` (and its `accessibilityLabel`) entirely; `RecoveryCard` (Home), `SleepCard`, `ReadinessCard`, `RecoverySignalsCard` gained a single `accessible`+`accessibilityRole="summary"` wrapper so a screen reader announces one coherent summary instead of fragmenting across child `Text` nodes |
| **Runtime/data-source verification** | No production data source changed — every fix wires an existing ViewModel method, application function, or persisted-state read path to a UI control that was missing it, or corrects a presentation-layer bug in front of data already read/written correctly; the two Profile fixes change only runtime-path *failure* behavior, not the `service`-based test/preview path or any other already-correct update method |
| **Tests** | New/updated: `HydrationCard.test.tsx`, `recovery-experience/__tests__/mapper.test.ts`, `recovery-experience/__tests__/runtimeIntegration.test.ts` (`changeDay` cross-day regression), `notification-center/components/__tests__/{NotificationCard,ReminderCard,NotificationSettingsCard,AddReminderRow}.test.tsx`, `notification-center/__tests__/reminderPresets.test.ts`, `progress-experience/__tests__/viewmodel.test.ts` (time-range blank-screen regression), `profile-experience/__tests__/{runtimeIntegration,screen}.test.tsx` (unsupported-fields-fail-clearly + live-theme-sync regressions); full suite (812 suites / 3369 tests) and typecheck remain green |
| **Design** | **UX completion + bug fixes only.** No runtime architecture change, no new runtime subsystem, no networking, no backend, no SQLite change, no Admin change, no live LLM provider, no new goal/notification persistence layer, no visual redesign — every new/changed component composes existing primitives (`AppCard`, `AppButton`, `Chip`, `ProgressBar`, `Ionicons`) |

Decision record: ADR-153 in [DECISIONS.md](./DECISIONS.md).
...
| Aspect | Implementation |
|--------|----------------|
| **Purpose** | First Phase B (Product Completion) sprint after Phase A (Production Hardening, Sprints 36.1–36.6) concluded. Audited the complete authenticated Expo Router route tree, main tab configuration, every production screen's screen → hook → runtime path, production `Mock*Service` usage, navigation actions, authentication boundaries, and refresh/restart behavior, with an explicit "fix only concrete navigation defects, no redesign" mandate |
| **Route tree audit** | Authenticated tree confirmed: 6-tab bar (`(tabs)/index,workout,nutrition,coach,progress,profile`) + secondary `goals`, `recovery` + `settings` → `settings/appearance` → `settings/appearance/theme` + `workout/{analytics,complete,detail,history,records,session,summary}`. Unauthenticated tree (`(auth)`, `(onboarding)`) and the `(app)/_layout.tsx` authenticated gate unchanged. No duplicate route files found |
| **Gap 1 — Notification Center fully built but unreachable** | `features/notification-center` (`NotificationCenterScreen`, `useNotifications` → hydrated Unified Workspace + Coach Timeline, Sprint 35.2) had zero Expo Router route mounting it and no UI entry point anywhere — a completely orphaned production feature |
| **Fix 1** | New route `app/(app)/notifications/index.tsx` mounts `NotificationCenterScreen`; new bell-icon `Pressable` (accessibility label `"Notifications"`) added to `HomeDashboardHeader`, rendered only when `onNotificationsPress` is supplied, wired in `HomeDashboardScreen` to `router.push("/(app)/notifications")` |
| **Gap 2 — Goals reachable in Coach quick actions but not from Home** | Goals is an activated production experience (`goal-progress-experience`, Sprint 35.0) with a route (`app/(app)/goals/index.tsx`) but no Home quick action pointed to it |
| **Fix 2** | New `VIEW_GOALS` quick action kind added to `DashboardProjectionQuickActionKinds` (production, `integrations/dashboard-projection`) and mirrored in `home/models/QuickAction.ts` (test/preview `mapQuickActions.ts` path) — destination `/(app)/goals`, `enabled: workspace.goals.present` |
| **Gap 3 — Recovery quick action / recommendation misrouted to Progress** | `mapWorkspaceToQuickActions.ts`'s `qa:view_recovery` action and `mapWorkspaceCoachToExperienceDto.ts`'s "Recovery recommendation" both had `destination: "/(app)/(tabs)/progress"` — tapping "Recovery" navigated to the Progress tab, not the Recovery screen |
| **Fix 3** | Both destinations corrected to `/(app)/recovery`; mirrored in the `coachExperienceData.ts` mock for consistency (test/preview data only) |
| **Gap 4 — always-visible buttons navigating to routes that don't exist** | Coach header's History/Settings buttons (`onHistoryPress`/`onSettingsPress` → `CoachHeader`), Coach insight-card detail taps, Workout `ExerciseCard`'s "Details" button, and every Progress analytics card's `onPress` all called `router.push()` with a `destination` string that has no matching file under `app/(app)/**` (e.g. `/(app)/coach/history`, `/(app)/progress/strength`, `/(app)/workout/exercise/[id]`) — these are the pre-existing, documented "navigation placeholder" pattern (PROJECT_STATE Sprint 27.1+) reaching production screens that actually wire `router.push`, not just unused DTO fields |
| **Fix 4 — `isReachableRoute` guard** | New pure utility `navigation/isReachableRoute.ts` holds a static allowlist mirroring the real `app/(app)/**` route tree; `isReachableRoute(destination)` returns `true` only for a destination that resolves to an existing route (query strings stripped before matching). `CoachExperienceScreen`, `WorkoutRuntimeScreen`, and `ProgressExperienceScreen` each compute navigation callbacks through a local `reachableHandler` that returns `undefined` for an unreachable destination instead of a function that calls `router.push()`; `CoachHeader` and `ExerciseCard` already conditionally render their action button only when the corresponding `onPress`/`onDetailsPress` prop is defined, so an unreachable destination now safely disables the button instead of navigating to Expo Router's not-found screen. `CoachInsightsCard`'s `onPress` prop was removed outright (its destinations are always unreachable placeholders) |
| **Gap 5 — Profile Appearance settings orphaned by the Sprint 31.6 rebuild** | `app/(app)/settings/index.tsx` → `settings/appearance` → `settings/appearance/theme` (working Light/Dark/System switching) had no link from `ProfileExperienceScreen` after it replaced the legacy `ProfileScreen`; the profile DTO's own `preferenceItems`/section destinations (e.g. `/(app)/profile/appearance`) don't correspond to any real route and are left as documented placeholders |
| **Fix 5** | `AppearanceCard` gained an optional `onPress` prop passed through to its underlying `AppCard`; `ProfileExperienceScreen` wires it directly to `router.push("/(app)/settings/appearance")` — the one placeholder destination in Profile that corresponds to an already-working, already-built screen |
| **Mock production audit** | No authenticated route or production hook defaults to a `Mock*Service` for Home, Profile, Workout, Nutrition, Recovery, Goals, Coach, or Notifications — all read hydrated runtime state per their Sprint 34.x/35.x activation (confirmed, no code change). `MockProgressAnalyticsService` remains the only working `ProgressAnalyticsService` implementation, preserved per this sprint's explicit instruction (documented pre-existing gap, Sprint 36.6/ADR-150) |
| **Legacy path audit** | `ProgressScreen.tsx`, `WorkoutScreen.tsx`, `DashboardScreen.tsx`, `NutritionScreen.tsx`, `CoachScreen.tsx`, `ProfileScreen.tsx` confirmed unreachable — zero production references, only self-referencing test files, not mounted anywhere in `app/app/**`. Left in place (not deleted); documented as safe-to-remove candidates for a future cleanup sprint, consistent with the two already documented in Sprint 36.6/ADR-150 (`useHome`, legacy `NutritionScreen`/`ProgressScreen`) |
| **Auth boundaries / refresh / restart** | Verified unchanged and correct (no code touched): unauthenticated → onboarding only; authenticated → authenticated tree; logout → full reset cascade → onboarding; direct navigation to an authenticated route while unauthenticated remains blocked by `app/(app)/_layout.tsx`; restart rehydrates via the existing SQLite/hydration pipeline with no mock fallback |
| **Tests** | New `navigation/__tests__/isReachableRoute.test.ts` (10 cases); new `coach-experience/__tests__/mapper.test.ts` (Recovery recommendation destination regression); extended `dashboard-projection/__tests__/mapper.test.ts` (Recovery quick-action destination + Goals quick-action gating) and `home/__tests__/screen.test.tsx` (Notifications entry point → `router.push` assertion); full existing suite (804 suites / 3342 tests) and typecheck remain green |
| **Design** | **Navigation wiring + routing-bug fixes only.** No runtime architecture change, no new runtime subsystem, no networking, no backend, no SQLite change, no Admin change, no live LLM provider, no visual redesign — only a bell icon and pressable-state changes on existing cards were touched, no new components/gradients/animations/typography |

Decision record: ADR-151 in [DECISIONS.md](./DECISIONS.md).

### Runtime Production Readiness Audit (`runtime/write-through`, `runtime/hydration`, `features/workout-runtime`, `features/athlete-identity`) — Sprint 36.6

| Aspect | Implementation |
|--------|----------------|
| **Purpose** | Final production-readiness audit of the complete runtime lifecycle (Bootstrap → Hydration → Dashboard Restore → Observer → Write-Through → Repository Contracts → SQLite → Restart → Hydration) before moving to product/UI completion. Every state machine, Composition Root lifecycle, athlete-scoping call site, observer wrap/unwrap path, persistence/hydration round trip, empty/populated startup path, logout/login cycle, failure UX surface, mock-usage boundary, and SQLite-access boundary was inspected; only concrete, provable defects were fixed |
| **Gap 1 — same-session concurrent persist completion** | Guard 2 (Sprint 36.5) rejects a stale `mutationSequence` only at `persist()`'s *entry*, before any repository write starts. Because `triggerWriteThrough()` resets write-through state and fires a new fire-and-forget `persist()` on every successful mutation without awaiting a prior call's repository I/O, two `persist()` calls can be genuinely concurrent (not just sequential): if an *older* mutation's call is still awaiting its repository writes when a *newer* mutation's call starts and finishes first, the entry check cannot see that — the older call already passed it before the newer one even began |
| **Fix 1 — post-write-through re-check** | `RuntimeWriteThroughPipeline.persist()` re-checks `isStaleWriteThroughSequence()` a second time immediately after `persistRuntimeRecords()` settles, before committing the `ready` state or calling `markWriteThroughSequenceApplied()`. A call that started fresh but finishes after a newer mutation already applied is rejected (`stale_mutation_sequence`) instead of reporting stale data as the current outcome; the `catch` block was also corrected so a stale rejection (at either check point) never downgrades an already-`ready` state back to `failed` |
| **Gap 2 — orphaned persist survives a full session teardown (logout)** | Gap 1's fix compares sequence *numbers*, but `RuntimeObserver.stop()` (called on every logout) also calls `resetRuntimeMutationSequence()`, zeroing both counters. An old in-flight `persist()` call from athlete A's just-logged-out session, captured at e.g. sequence 5, can complete *after* logout resets the counters to 0 and athlete B's new session has already applied its own sequence 1 — `5` is not `< 1`, so the numeric guard alone would incorrectly accept it as "fresh" and let it commit state/repository writes that belong to a torn-down session |
| **Fix 2 — session epoch guard** | New `sessionEpoch` counter in `RuntimeWriteThroughSequence.ts` — never reset to zero, only ever incremented, and bumped exactly where `resetRuntimeMutationSequence()` already runs (i.e. only on a genuine `RuntimeObserver.stop()` session boundary, never on the routine per-mutation `triggerWriteThrough()` reset). `persist()` captures `getWriteThroughEpoch()` at entry (no new public option; purely internal) and re-checks it alongside the sequence after I/O settles; a mismatch rejects with a new `RuntimeWriteThroughError` code `stale_write_through_epoch`, which is excluded from the failed-state downgrade exactly like `stale_mutation_sequence` |
| **Gap 3 — workout timer performance hotspot** | `WorkoutRuntimeViewModel.tickRestTimer()` / `tickDuration()` (driven by a 1s `setInterval` for the entire duration of an active workout) called `notify()`, which unconditionally triggers `persistIfRuntimeDriven()` → observer-wrapped `build()` → full cross-domain write-through → synchronous SQLite write, every single tick |
| **Fix 3 — UI-only notify path** | New private `notifyListeners()` on `WorkoutRuntimeViewModel` notifies UI subscribers without persisting; `tickRestTimer()`/`tickDuration()` now call it instead of `notify()`. Every other mutation (set completion, navigation, finish, dialogs) is unchanged and still persists via `notify()` — only the two per-second timer ticks stop triggering write-through |
| **Gap 4 — stale observer metadata** | `RUNTIME_OBSERVER_WATCHED_SERVICES` (default `RuntimeObserverResult.watchedServices`) listed only the original three services from Sprint 33.7, silently omitting the three domain-persistence services (`WorkoutRuntimePersistenceService`, `NutritionRuntimePersistenceService`, `RecoveryRuntimePersistenceService`) added in Sprint 35.4 — `RuntimeObserver.start()` has wrapped all six since 35.4, but its own reported result under-stated what it actually observes |
| **Fix 4 — corrected constant** | `RUNTIME_OBSERVER_WATCHED_SERVICES` now lists all six actually-wrapped services; purely a metadata correction, no behavior change (nothing wraps/unwraps differently) |
| **Gap 5 — identity hydration round trip loses the identity's own bookkeeping fields** | Auditing persistence/hydration round trips domain-by-domain (audit item 6) found that `restoreIdentityRecords()` (`runtime/hydration/HydrationRestoration.ts`) called `AthleteIdentityService.build()` with the persisted `profile`/`preferences`/`settings`/`locale`/`units`/`timeZone` but a fresh hydration `requestId` and clock reading — `build()` always derives `id`, `createdAt`, and `metadata` (`generatedAt`/`identityId`/`version`/`schemaVersion`) from its own inputs, so those four fields were silently regenerated on every restart instead of round-tripping the persisted values, unlike every other domain (Workspace/Workout/Nutrition/Recovery/Snapshot/Timeline), which all already restore via a dedicated `restorePersisted()` that installs the persisted value verbatim |
| **Fix 5 — `AthleteIdentityService.restorePersisted()`** | Added `restorePersisted(identity: AthleteIdentity): void` to `AthleteIdentityService`, mirroring the exact pattern already used by the other six domain services (installs the identity into `latestByAthlete` verbatim, with the same `identityIds` bookkeeping `build()` performs on success); `restoreIdentityRecords()` now calls it instead of `build()`. `build()`'s own contract (fresh `id`/`createdAt`/`metadata` on every real mutation) is unchanged — this only changes what hydration does |
| **Verified clean, no code change** | Runtime state machine determinism (idle resets, no permanently-stuck in-progress state, no duplicate wrapping on repeated start/logout/login); Composition Root singleton lifecycle and reset-on-logout; athlete-id threading from `AuthContext.user.id` through session → hydrate → restore → observe → every feature screen, with no cached id surviving logout; observer wrap-once/unwrap-on-failure/unwrap-on-stop semantics; persistence/hydration semantic round trips for Workout, Nutrition, Recovery, Goal Progress, Coach, Notification, and Workspace (Identity's own round trip is Gap 5/Fix 5 above — its `profile`/`preferences`/`settings`/`locale`/`units`/`timeZone` payload always round-tripped correctly); empty- and populated-first-start behavior for all eight feature areas (Coach's "no workspace" path was already covered by an existing test asserting a graceful `coach_runtime_unavailable` error state, not a crash); full A → logout → B → logout → A isolation, including observer/write-through/hydration/dashboard/bootstrap/session state all returning to idle; the single `RuntimeFailureScreen` failure funnel with no raw exceptions surfaced and no second retry mechanism; no authenticated production screen defaulting to a `Mock*Service` (two legacy hooks/screens found still wired to mock-by-default factories — `useHome`, `screens/NutritionScreen.tsx`, `screens/ProgressScreen.tsx` — are confirmed unreachable dead code, not mounted anywhere in the route tree); `MockProgressAnalyticsService` is knowingly the only working `ProgressAnalyticsService` implementation (the `backend`/`local` providers are documented stubs that throw `notConfigured()`), so its production use is an existing, intentional architectural gap rather than an accidental fallback; no feature/application/runtime code imports `expo-sqlite`/`SQLiteEngine`/`SQLiteConnection`/`SQLiteRepositories` outside `infrastructure/sqlite` and `core/composition` |
| **Composition Root** | None — no new services, tokens, or factories |
| **Design** | **Hardening + one narrow performance correction only.** No runtime architecture redesign, no new runtime subsystem, no debounce/batching/background worker/queue, no networking, no backend, no SQLite infrastructure changes, no Admin changes, no AI/Coach changes, no visual redesign |

Decision record: ADR-150 in [DECISIONS.md](./DECISIONS.md).
...
| **Coach** | `CoachRuntimePersistenceState` stored on `Workspace.coachRuntimeOverlay`; conversation turns + Conversation Memory entries; persisted through existing `WorkspaceRepository` write-through (no Coach repository) |
| **Notifications** | `NotificationRuntimePersistenceState` stored on `Workspace.notificationRuntimeOverlay`; read/unread, settings, reminder overlays; persisted through existing `WorkspaceRepository` write-through; dismiss/create also journaled to Coach Timeline |
| **Design** | **No new persistence architecture.** Reuses Workout/Nutrition/Recovery repositories and Unified Workspace boundary for goals, coach, and notifications. No direct SQLite from feature layers. Runtime Session unchanged. Minimal Runtime Observer extension wraps domain persistence service `build()` hooks. |
