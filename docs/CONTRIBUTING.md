...
This document defines how work gets done in the EVOLVE repository: branching, commits, pull requests, code review, and how to use Cursor effectively on this codebase.

For architectural context, read [`EVOLVE_ARCHITECTURE.md`](../.cursor/rules/EVOLVE_ARCHITECTURE.md) and the Cursor rules in [`evolve.mdc`](../.cursor/rules/evolve.mdc) before contributing.
...
7. **Update documentation alongside code.** Ask the agent to update `ROADMAP.md`, `TASKS.md`, and `CHANGELOG.md` as part of the same change when relevant.
