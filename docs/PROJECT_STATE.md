...
| Navigation | 6-tab bottom bar (Home, Workout, Nutrition, Coach, Progress, Profile) + secondary routes Recovery/Goals + Notifications (bell icon on Home, `app/(app)/notifications`, Sprint 37.1) + Settings → Appearance → Theme (linked from Profile, Sprint 37.1) |
...
| Backend providers | `BackendUserService`, `BackendWorkoutService` live; `BackendProfileExperienceService` live for backend-owned fields only (`getProfile`/`updateAthleteInfo` via `GET`/`PATCH /api/v1/users/me`, Phase A.1); other `Backend*Service` classes throw `notConfigured()` |
| Tests | Jest + jest-expo |
| Sprint status | Sprint 38.3 First-Run Runtime Bootstrap shipped (narrow production-blocker fix: a newly registered athlete could authenticate and reach Runtime Session `ready`, but Athlete Identity and Unified Workspace were never composed — hydration of 0 records succeeded, Dashboard Restore fell back to `createEmptyHomeDashboard()`, and Profile/feature screens later failed with "Athlete identity unavailable"; `initializeFirstRunRuntime()` now runs inside `RuntimeSessionOrchestrator` after hydration and before dashboard restore, calling the existing `composeAthleteIdentity` / `composeUnifiedWorkspace` application APIs only when those records are missing, persisting through `persistRuntime()`, and threading `identitySeed` from the current `AuthContext` user — athlete id still originates exclusively from `AuthContext.user.id`; first-run failures use the existing `failed` state + `RuntimeFailureScreen` + `retrySession()`; no Runtime/SQLite/Observer/Write-Through/repository redesign, no networking, no new endpoints, no Admin/AI/UI redesign; new `firstRunBootstrap.integration.test.tsx`; full suite 820/3465 and typecheck green; see ADR-156); Phase A.1 Profile Mobile ↔ Backend Integration shipped (Phase A — Backend Infrastructure started; connected `ProfileExperienceService`'s previously-throwing `backend` provider to the already-implemented `GET`/`PATCH /api/v1/users/me`, reusing the existing authenticated API client and `UserPublic`/`UserUpdate` schemas with no new networking/backend/persistence layer; audited `/api/v1/users/me` against the Profile Experience read model and confirmed it owns only name/email/birth-date/height/weight/activity-level/goal/account-status — none of training/nutrition/coach/notification/appearance/units/goal-progress preferences, which stay explicitly unsupported (now with an accurate "not supported by the backend user API yet" message); new `mapUserPublicToProfileDto` mapper + `ProfileExperienceService.updateAthleteInfo()` contract method + `application/UpdateAthleteInfo.ts` + `ProfileExperienceViewModel.updateAthleteInfo()` wire the one previously-unconnected backend-owned field pair, `heightCm`/`weightKg` (hardcoded `null` in `mapAthleteIdentityToProfile` since Athlete Identity has no such field) — the production runtime path now calls `PATCH /api/v1/users/me` directly for them (no SQLite/Runtime Observer/Athlete Identity model change) and merges the response into the hydrated profile in memory, propagating failures as `ProfileExperienceError` without ever reporting a save that didn't happen; no UI changes; full suite (811 suites / 3374 tests) and typecheck green; see ADR-155); Sprint 37.4 Production Runtime Validation & Legacy Cleanup shipped (Phase B — Product Completion continued; a final validation-and-cleanup sprint re-auditing the complete production lifecycle — Auth → Runtime Session → Hydration → Dashboard Restore → Observer → Persistence → Feature UI → Logout → different-athlete login → Restart — before Backend/Admin/AI infrastructure work begins, under an explicit "verify first, remove only what's proven dead" mandate; re-walked the full Expo Router tree (route audit) and confirmed every authenticated route resolves to a real screen, documenting `workout/records`/`workout/session`/`workout/summary` as intentionally-inert route files (no in-app navigation trigger, but never named in this sprint's cleanup list and each backed by its own passing tests) rather than deleting them; re-confirmed the six legacy screens Sprint 37.1 flagged (`ProgressScreen.tsx`, `WorkoutScreen.tsx`, `DashboardScreen.tsx`, `NutritionScreen.tsx`, `CoachScreen.tsx`, `ProfileScreen.tsx`) still have zero production references and deleted all six plus their two dedicated test files (`ProfileScreen.test.tsx`, `CoachScreen.test.tsx`), while deliberately leaving their now-orphaned transitive dependencies in place (each still has its own independent test suite and/or live sibling exports still used by production, e.g. `MockProgressService` behind `progress-experience`, nutrition domain models behind `integrations/nutrition-progress`, `SettingsRow` behind Settings/Appearance/Theme) as a materially larger, separately-scoped future cleanup; a Mock/demo audit across the full authenticated route tree found zero production-reachable Mock/demo/fake data (every production screen's hook/ViewModel takes the hydrated-runtime path with no injected `service`; `MockProgressAnalyticsService` and the Sprint 30.x Mock adapters reconfirmed as intentional production infrastructure); a read-only re-verification of runtime lifecycle, athlete isolation, persistence/restart, failure recovery, and write-through concurrency against current code (with file/line evidence) found all nine areas — startup order, `retrySession()`'s unconditional full reset, the logout reset cascade, hydration's athlete-scope filter and payload-ownership guard, all eight domains' SQLite round-trip, `triggerWriteThrough()`'s `.catch()`, and the write-through concurrency guards (pre-/post-I/O stale-sequence checks, `sessionEpoch`, `stale_write_through_epoch`, partial-wrap cleanup) — already intact, so zero runtime/persistence code was changed; a performance audit confirmed the Sprint 36.6 rest-timer fix remains intact (ticks call only `notifyListeners()`, never write-through) and found no polling anywhere in Progress/Coach/Notifications; an accessibility audit of the Sprint 37.2/37.3 changes found and closed three narrow regressions with no visual change — `AppButton` gained an additive opt-in `selected?: boolean` prop so `TimeRangeSelector` can expose the active time range via `accessibilityState.selected` (previously undetectable to a screen reader), `NotificationCard`'s accessible label now includes priority when its high-priority `Chip` is shown, and `GoalsCard`/`NotificationPreferencesCard` (pressable since 37.3) gained explicit `accessibilityLabel`s describing their navigation destination; full suite (810 suites / 3354 tests — down from 812/3369, exactly the two removed legacy-screen test files) and typecheck remain green; no runtime architecture change, no new runtime subsystem, no networking, no backend, no SQLite/schema change, no Admin, no live LLM, no repository contract change, no visual redesign; see ADR-154); Sprint 37.3 Production UX Completion & Feature Surface Polish shipped (Phase B — Product Completion continued; a functional-completeness pass over the ten production screens (Home, Workout, Nutrition, Recovery, Goals, Coach, Progress, Profile, Notifications, Settings) auditing primary/secondary actions, empty/loading/error states, save/update feedback, disabled states, and accessibility; closed concrete gaps by wiring existing-but-unwired ViewModel/application capabilities rather than inventing new domain behavior: Workout gained a `WorkoutCompleteCard` final state (replacing active editors once `runtime.state.isCompleted`) and a header "Completed" `Chip`; Nutrition's `HydrationCard` gained a working Log button wired to the existing `logHydration()`; Recovery's day navigation was fixed (`buildAvailableDays()` now shifts real ISO dates via a new `shiftIsoDate()` helper instead of cloning one date onto every option, and `changeDay()` no longer re-passes the current day's in-memory values as overrides that stomped the target day's own persisted state); Coach gained a compact `CoachInlineError` for send/regenerate failures (conversation history no longer disappears behind a full-page error) and corrected "Streaming response…" to "Coach is responding…" to stop implying live LLM streaming; Notifications wired `markRead`/`editReminder`/`removeReminder`/`updateSettings` (previously unreachable from the UI) and added a `REMINDER_PRESETS`/`AddReminderRow` quick-add affordance closing the reminder-creation gap; Progress fixed a blank-screen flash on every time-range switch (`showContent` no longer requires `!isLoading` against a dashboard the ViewModel already keeps populated during reload) and added a `disabled` guard to `TimeRangeSelector`; Profile fixed the sprint's most significant defect — `updateGoals()`/`updateNotificationPreferences()`'s runtime path silently returned the athlete's profile unchanged while reporting success, since goals/notification prefs are owned by other features with no Athlete Identity field to persist into — both now throw a clear `ProfileExperienceError` instead of silently pretending to save; `GoalsCard`/`NotificationPreferencesCard` now link out to their owning tabs instead of showing static (and in Notifications' case, entirely hardcoded-false) duplicate state; `AppearanceCard` now shows the live `ThemeContext` preference instead of a `settings.appearance` identity field the real theme switch never writes to; `TrainingPreferencesCard`/`NutritionPreferencesCard` now render "—" instead of misleading hardcoded `0`s for fields with no identity backing; accessibility fixes closed gaps in `AppButton` (label lost during loading), `Chip` (disabled state stripped its label entirely), and four Recovery/Home info cards (no single accessible summary); no runtime/persistence/SQLite architecture change, no new networking/backend/Admin/live LLM, no new goal/notification persistence layer, no visual redesign; full suite (812 suites / 3369 tests) and typecheck green; see ADR-153); Sprint 37.2 Premium Visual Experience shipped (Phase B — Product Completion continued; a visual/product polish pass over the ten production screens surfaced by Sprint 37.1's navigation audit — Home, Workout, Nutrition, Recovery, Goals, Coach, Progress, Profile, Notifications, Settings — reusing existing design-system primitives (`AppCard`, `AppButton`, `ProgressBar`, `SectionTitle`, `FloatingSurface`, `Chip`) with no new visual language; Home gained staggered per-section entrance animation (`DashboardSection` now accepts an `index` stagger position); Workout's bottom action bar now renders on `FloatingSurface` for a floating-glass footer consistent with its rest-timer card, set rows show a completed-set checkmark, the active exercise gained an "Current exercise" eyebrow label, and the rest timer gained a progress bar; Recovery's `ReadinessCard`/`SleepCard`/`RecoverySignalsCard` were redesigned with icon rings, metric typography, and `AppButton` actions in place of plain text links; Coach's send control became an icon button; Profile's `AppearanceCard` gained a theme icon and chevron affordance; Notifications' `NotificationCard` was redesigned with an icon ring, unread indicator, priority `Chip`, and accessible dismiss control, and its "Reminders" section now uses `SectionTitle`; six mojibake `?` separators in Progress chart cards were corrected to `·`; day selectors (Nutrition/Recovery) and goal/milestone actions gained `accessibilityRole`/`accessibilityState`/`accessibilityLabel`; no runtime/persistence/navigation/data-source changes — Home/Profile/Workout/Nutrition/Recovery/Goals/Coach/Notifications/Progress continue reading exclusively from their existing hydrated/persisted production sources; no new networking, backend, Admin, or live LLM provider; Settings/Appearance/Theme left unchanged as already consistent and appropriately minimal; see ADR-152); Sprint 37.1 Production UI & Navigation Completion Audit shipped (Phase B — Product Completion started; audited whether every activated production experience — Home, Profile, Workout, Nutrition, Recovery, Goals, Coach, Notifications, Progress — is actually reachable through the production Expo Router tree and uses its correct runtime path; fixed five concrete navigation defects: wired the fully-built-but-unreachable Notification Center into a new `app/(app)/notifications` route with a new bell-icon entry point on Home; added a missing Home quick action for Goals; fixed a real routing bug sending "Recovery" quick action/recommendation taps to the Progress tab instead of Recovery; added a shared `navigation/isReachableRoute.ts` allowlist guard so Coach's History/Settings buttons, Coach insight-card taps, Workout's exercise "Details" button, and Progress's analytics cards disable themselves instead of navigating to non-existent routes; wired Profile's orphaned Appearance card to the already-working Settings → Appearance → Theme flow; confirmed six legacy screens (`ProgressScreen`/`WorkoutScreen`/`DashboardScreen`/`NutritionScreen`/`CoachScreen`/`ProfileScreen`) as unreachable dead code and documented them without deleting; no runtime architecture change, no mock-service regressions, no visual redesign; see ADR-151); Sprint 36.6 Runtime Production Readiness Audit shipped (Phase A — Production Hardening concluded; the final runtime-hardening audit before moving to product/UI completion, covering every stage of the lifecycle — Bootstrap → Hydration → Dashboard Restore → Observer → Write-Through → Repository Contracts → SQLite → Restart → Hydration — under an explicit "fix only concrete defects" mandate; found and closed five narrow defects with the smallest possible corrections: (1) Sprint 36.5's mutation-sequence guard only rejected a stale write-through call at `persist()`'s entry, which cannot catch two genuinely concurrent `persist()` calls (the fire-and-forget `triggerWriteThrough()` pattern never awaits a prior call before starting a new one) — a second `isStaleWriteThroughSequence()` check now runs immediately after `persist()`'s own repository writes settle, before committing the `ready` state, so a slow older mutation's late-arriving result can no longer silently overwrite a faster newer mutation's already-applied outcome; (2) that fix alone still could not catch an orphaned `persist()` surviving a full session teardown (logout), because `RuntimeObserver.stop()` zeroes the sequence counters themselves, making an old in-flight call's captured sequence look numerically "fresh" against a brand-new session's own low counters — a new never-reset `sessionEpoch` counter (`RuntimeWriteThroughSequence.ts`), bumped only on that same genuine session boundary (never the routine per-mutation reset), closes this gap with a new error code `stale_write_through_epoch`; (3) `WorkoutRuntimeViewModel`'s rest-timer/duration ticks (driven by a 1-second timer for the entire duration of an active workout) were triggering a full cross-domain SQLite write-through every single second — a new UI-only notify path stops this while every other real mutation still persists exactly as before; (4) the Runtime Observer's own reported `watchedServices` metadata had silently drifted out of sync since Sprint 35.4 (still listing only 3 of the 6 services it actually wraps) and was corrected; (5) auditing persistence/hydration round trips domain-by-domain found that identity hydration (`restoreIdentityRecords()`) restored a persisted identity via `AthleteIdentityService.build()`, which always derives a fresh `id`/`createdAt`/`metadata` from its own inputs, silently regenerating an athlete's identity's own bookkeeping fields on every restart even though `profile`/`preferences`/`settings`/`locale`/`units`/`timeZone` round-tripped correctly — added `AthleteIdentityService.restorePersisted()`, mirroring the exact existing pattern already used by every other domain service (Workspace/Workout/Nutrition/Recovery/Snapshot/Timeline), and switched hydration to call it instead of `build()`; every other audited area — runtime state machine determinism, Composition Root lifecycle, athlete-id scoping/isolation end-to-end, observer wrap/unwrap semantics, the rest of the persistence/hydration round trips across the remaining seven domains, empty/populated first-start behavior across all eight feature areas, full cross-athlete logout/login isolation, the single `RuntimeFailureScreen` failure funnel, production mock-usage boundaries, and direct-SQLite-access boundaries — was verified already correct with no code change; no new runtime subsystem, no runtime architecture redesign, no debounce/batching/background worker/queue, no networking, no backend, no SQLite infrastructure changes, no Admin changes, no AI/Coach changes, no visual redesign; see ADR-150); Sprint 36.5 Runtime Persistence Verification & Consistency Guard shipped (Phase A — Production Hardening continued; a verification pass over the complete persistence lifecycle — Runtime Session → Observer → Domain Mutation → Write-Through → Repository Contracts → SQLite → Hydration → Restore — found two concrete gaps and closed both with the smallest possible pure guards, per the sprint's explicit "verify, don't rewrite" mandate: (1) a payload-internal athlete-ownership check (`runtime/persistence/AthleteRecordOwnership.ts`) now runs inside every write-through `observe*Records()` and hydration `restore*Records()` function, catching a record legitimately keyed under the current athlete whose payload was itself built for a different `athleteId` — a case the Sprint 36.1 `filterRecordsForAthleteScope()` (`record.id`-only) check could not catch; a mismatch is skipped and logged, never persisted/restored (`AthleteSnapshot`'s composite-id keying is documented as an explicit, out-of-scope exception); (2) a monotonic mutation-sequence counter (`runtime/write-through/RuntimeWriteThroughSequence.ts`) now lets `RuntimeWriteThroughPipeline.persist()` reject — before touching any service or repository — a stale/out-of-order write-through call whose captured mutation predates one already applied, guaranteeing "latest mutation wins" even if an earlier mutation's persist call happens to complete after a later one's; both guards are pure functions/counters wired into existing call sites with one new optional field each (`mutationSequence` on `PersistRuntimeOptions`/`RuntimeWriteThroughOptions`) and one new `RuntimeWriteThroughError` code (`stale_mutation_sequence`) — no event sourcing, no versioned event log, no background worker, no queue, no retry system, no network sync, no new repository; mutation/failure sequencing (A→persist, B→persist, C→fails, D→persist — only D survives) and persistence-failure consistency (Sprint 36.4) were re-verified and found already correct — no code change; athlete isolation (A→logout→B→logout→A, Sprint 36.1) verified extended across every cross-domain overlay (Workout/Nutrition/Recovery/Goal/Coach/Notification) in one new integration test; new cross-domain persist+restart, post-hydration dashboard-vs-mock-fallback, and full-reset state-inventory (every pipeline's status **and** result, observer `athleteIds`, both new sequence counters) integration tests close testing gaps with no production-code change; no new runtime subsystem, no SQLite/networking/backend/Admin/AI changes, no visual redesign; see ADR-149); Sprint 36.4 Runtime Persistence Failure Recovery shipped (Phase A — Production Hardening continued; a repository/write-through failure automatically triggered by the Runtime Observer is now fully deterministic — `RuntimeObserver`'s `triggerWriteThrough()` catches the `persistRuntime()` rejection instead of letting it become an unhandled promise rejection, logging the reason via the existing `infrastructure/logging` abstraction; the failure remains fully observable through the existing `RuntimeWriteThroughState`/`getWriteThroughStatus()` status+error mechanism without ever rolling back the in-memory runtime mutation that triggered it; Runtime Observer and Runtime Session both remain `"ready"` — persistence failure is confirmed as a concern fully separate from runtime execution/session availability; recovery on the next successful mutation was already structurally guaranteed by the observer's existing reset-before-every-persist pattern (`resetRuntimeWriteThrough()` then `persistRuntime()` on every successful `build()`, not only after a failure) — verified rather than re-implemented; "latest mutation wins" and full domain coverage (Workout, Nutrition, Recovery, Goal Progress, Coach, Notification) proved across repository-failure + recovery + restart/rehydration scenarios; logout's existing reset cascade (Sprint 36.3) already clears write-through state unconditionally, so no stale persistence-failure state survives logout; no new runtime subsystem, no retry queue/background worker/debounce/batching/event bus, no new repository/provider, no SQLite/networking/Admin/AI changes, no visual redesign; see ADR-148); Sprint 36.3 Runtime Session Retry Hardening & Recovery State Reset shipped (Phase A — Production Hardening continued; `retrySession()` now performs its full deterministic pipeline reset unconditionally — Observer → Write-Through → Dashboard Restore → Hydration → Bootstrap → Session — instead of gating the reset on the orchestrator's internal status, closing a gap where an observer-only failure left stale pipeline state in place across a retry; `resetRuntimeWriteThrough()` added to the shared reset cascade; `RuntimeObserver.start()` now unwinds partially-wrapped `build()` methods on a mid-sequence failure, preventing duplicate wrapping/double-persist on a later successful start; athlete isolation reaffirmed — retry always reuses the current authenticated `user.id`, never a cached/hydrated/persisted athlete id; `ErrorBoundary` remains independent of Runtime Session retry; no new runtime subsystem, no persistence/SQLite/networking/admin/AI changes, no visual redesign; see ADR-147); Sprint 36.2 Global Runtime Failure UX & Error Boundary shipped (Phase A — Production Hardening continued; authenticated startup failures — bootstrap, hydration, dashboard restore, or observer — now render a dedicated global `RuntimeFailureScreen` instead of leaving authenticated navigation reachable with no recovery surface; `app/(app)/_layout.tsx` blocks the authenticated `Stack` while Runtime Session `status === "failed"`; Retry calls the existing `retrySession()` — no parallel retry mechanism; raw exceptions/SQLite internals/stack traces are never shown to the user; new reusable `ErrorBoundary` wraps only the authenticated `Stack` (auth/onboarding unaffected) and catches unexpected React render exceptions with a safe "Try again" fallback; both surfaces log through the existing `infrastructure/logging` abstraction (Sprint 30.6), no `console.*` production path; no new runtime subsystem, no persistence/SQLite/networking/admin/AI changes, no visual redesign; see ADR-146); Sprint 36.1 SQLite Session Data Isolation / Logout Hardening shipped ( closed production data-isolation gap where SQLite data survived logout and could be rehydrated by a different athlete's next login; `RepositoryHydrationPipeline` now accepts `athleteIds` and filters athlete-owned records through new `filterRecordsForAthleteScope()` Authenticated Athlete Persistence Boundary before restoring into runtime memory; `RuntimeSessionOrchestrator` threads authenticated session `athleteIds` into hydration; SQLite retained on disk (no destructive wipe) so a returning athlete recovers only their own data; Repository Contracts, SQLite infrastructure, Composition Root, Runtime Session/Observer/Bootstrap public APIs unchanged; see ADR-145); Sprint 35.5 Coach & Notification Persistence Completion shipped (Coach conversation turns + Conversation Memory entries persist through `Workspace.coachRuntimeOverlay` + existing WorkspaceRepository write-through; Notification read/settings/reminder overlays persist through `Workspace.notificationRuntimeOverlay`; dismiss/create also journaled to Coach Timeline; hydration restores overlays and Conversation Memory via `loadHydratedCoachExperience()` / `loadHydratedNotificationExperience()`; no new repositories; no push/network; transient Coach UI pin/dismiss remains session-local; Runtime Session unchanged); Sprint 35.4 Domain Runtime Persistence Completion shipped (Workout/Nutrition/Recovery in-session mutations persist through existing Workout/Nutrition/Recovery repository contracts + domain serializers; Goal Progress milestone/completion overlay persists through `Workspace.goalRuntimeOverlay` + existing WorkspaceRepository write-through; Runtime Observer minimally extended to wrap domain persistence service `build()`; hydration restores overlays via `loadHydrated*()`; no new persistence architecture; no direct SQLite from feature layers; Runtime Session unchanged); Sprint 35.3 Progress & Analytics Runtime Activation shipped (Progress tab production path reads Progress Analytics read models via `applyHydratedProgress()`; default Mock ProgressExperienceService loading removed from authenticated startup; refresh/time-range changes re-load analytics and reproject timeline where supported; ProgressExperienceService retained for test/preview; no duplicate analytics engine; Backend/Local ProgressAnalyticsService providers remain stubs — in-memory read model fed by Sprints 32.1–32.4 runtime integrations); Sprint 35.2 Notification Runtime Activation shipped (Notification Center production path reads hydrated Unified Workspace + Coach Timeline via `applyHydratedNotifications()`; default Mock NotificationCenterService loading removed from authenticated startup; dismiss/reminder mutations append Coach Timeline lifecycle events where supported; NotificationCenterService retained for test/preview; no push/network — Expo/FCM/APNS deferred); Sprint 35.1 Coach Runtime Activation shipped (Coach tab driven by hydrated Unified Workspace coach projection + existing Coach Conversation orchestration; `applyHydratedCoachExperience()` is production source of truth; Mock CoachExperienceService loading removed from startup; coaching turns execute via `processCoachConversationTurn` through Conversation Memory, Agent Collaboration, and Coach Timeline append where supported; CoachExperienceService retained for test/preview; no live LLM provider — deterministic orchestration seam); Sprint 35.0 Goal Progress Runtime Activation shipped (Goal Progress screen driven by hydrated Unified Workspace goals projection; `applyHydratedGoalProgress()` is production source of truth; Mock GoalProgressExperienceService loading removed from startup; progress/milestone/completion mutations publish through existing Sprint 32.4 integration; GoalProgressExperienceService retained for test/preview); Sprint 34.9 Recovery Runtime Activation shipped (Recovery screen driven by hydrated Unified Workspace recovery projection; `applyHydratedRecovery()` is production source of truth; Mock RecoveryExperienceService loading removed from startup; sleep/readiness/assessment mutations publish through existing Sprint 32.3 integration; RecoveryExperienceService retained for test/preview); Sprint 34.8 Nutrition Runtime Activation shipped (Nutrition tab driven by hydrated Unified Workspace + Plan History nutrition plan; `applyHydratedDashboard()` is production source of truth; Mock NutritionExperienceService loading removed from startup; meal/hydration mutations publish through existing Sprint 32.2 integration; NutritionExperienceService retained for test/preview); Sprint 34.7 Workout Runtime Activation shipped (Workout tab driven by hydrated Unified Workspace + Workout Assembly cache; `applyHydratedWorkout()` is production source of truth; Mock WorkoutRuntimeExperienceService loading removed from startup; finish publishes WorkoutCompleted via existing Sprint 32.1 integration; WorkoutRuntimeExperienceService retained for test/preview); Sprint 34.6 Profile Update Persistence shipped (supported Profile edits persist via AthleteIdentityService.build() → Runtime Observer → Write-Through → SQLite; unsupported fields unchanged; ProfileExperienceService retained for test/preview); Sprint 34.5 Real Profile & Identity Persistence shipped (Profile screen driven by hydrated AthleteIdentityService; `applyHydratedProfile()` is production source of truth; Mock ProfileExperienceService loading removed from startup); Sprint 34.4 Real Home Dashboard Activation shipped (Home screen driven exclusively by Dashboard Restore; `applyRestoredDashboard()` is production source of truth; HomeService mock loading removed from startup; tests/previews retain injectable HomeService); Sprint 34.3 Domain Persistence Serialization shipped (full immutable domain JSON in SQLite payload; repository-layer serializers; hydration/write-through restore complete domain models; PersistenceRecord contracts unchanged); Sprint 34.2 Native SQLite Engine Integration shipped (Expo SQLite persistent driver; automatic schema init; database survives restart; Infrastructure-only change); Sprint 34.1 SQLite Runtime Persistence Activation shipped (runtime hydration/write-through use SQLite repository adapters via PersistenceRepositoryProvider; Phase 34 started); Sprint 33.9 End-to-End Runtime Persistence Validation shipped (full lifecycle integration tests; User Story 01 validated); Sprint 33.8 Runtime Auto-Start Wiring shipped (observer wired to session lifecycle); Sprint 33.7 Runtime Change Observer shipped (automatic persistence); Sprint 33.6 Runtime Startup Integration shipped (User Story 01 orchestration complete); Sprint 33.5 Runtime Session Orchestrator shipped (Phase 33 in progress); Sprint 33.4 Runtime Write-Through Pipeline shipped; Sprint 33.3 Dashboard Restore Pipeline shipped; Sprint 33.2 Repository Hydration Pipeline shipped; Sprint 33.1B Runtime Bootstrap Gate shipped; Sprint 32.6 Unified Workspace → Dashboard Projection shipped (Phase 32 complete); Sprint 32.5 Progress Analytics → Coach Timeline Projection shipped; Sprint 32.4 Goal Progress → Progress Analytics Integration shipped; Sprint 32.3 Recovery → Progress Analytics Integration shipped; Sprint 32.2 Nutrition → Progress Analytics Integration shipped; Sprint 32.1 Workout → Progress Analytics Integration shipped; Sprint 31.9 Coach Timeline Framework shipped; Sprint 31.6 Profile & Settings Experience shipped; Sprint 31.5 Nutrition Experience shipped; Sprint 31.4 Progress & Analytics Experience shipped; Sprint 31.3 AI Coach Experience shipped; Sprint 31.2 Workout Runtime Experience shipped; Sprint 31.1 Real Home Dashboard shipped; History + detail (13.1–13.2); workout analytics (14.0); AI workout pipeline through Training Adaptation (17.1–17.5) |
...
- Persist only through existing `persistRuntime({ athleteIds })` when anything was created; skip when bootstrap is not actually `ready` or when the records already exist
- Athlete id still originates exclusively from `AuthContext.user.id`; A → logout → B isolation re-verified (B is first-run initialized under B's id; A's rows are not hydrated into B)
- First-run composition/persist failure throws `RuntimeSessionError(..., "initialization_failed")`, session enters the existing `failed` state, `RuntimeFailureScreen` remains the UX, Retry uses the existing `retrySession()` full pipeline reset
- Tests: new `firstRunBootstrap.integration.test.tsx` (brand-new success, existing athlete unchanged, identity/workspace created only when missing, A→B isolation, failure UX, retry after failure); existing lifecycle/isolation/startup/retry/profile/sqlite assertions updated where they assumed empty first-start
- `npm test -- --no-coverage` (820 suites / 3465 tests) and `npm run typecheck` both pass
- No Runtime/SQLite/Observer/Write-Through/repository redesign, no networking, no new backend endpoints, no Admin, no AI, no UI redesign; ADR-156; docs: ARCHITECTURE, PROJECT_STATE, CHANGELOG, DECISIONS

Previous: **37.4 — Production Runtime Validation & Legacy Cleanup** (2026-08-12)

- Final validation-and-cleanup sprint before Backend/Admin/AI infrastructure work begins — re-audits the complete production lifecycle (Auth → Runtime Session → Hydration → Dashboard Restore → Observer → Persistence → Feature UI → Logout → different-athlete login → Restart) end-to-end after three consecutive UI-focused sprints (37.1–37.3), under an explicit "verify first, remove only what's proven dead, no redesign" mandate
- **Route audit:** re-walked the full Expo Router tree; every authenticated route (6-tab bar, `goals`, `recovery`, `notifications`, `settings` → `appearance` → `theme`, `workout/{detail,history,analytics,records,session,summary,complete}`) resolves to a real screen component. `workout/history`→`workout/detail` and `workout/analytics` are reachable via `WorkoutRuntimeScreen`'s header buttons through the existing `isReachableRoute.ts` allowlist. `workout/records`, `workout/session`, and `workout/summary` remain route files with **no in-app navigation trigger** (the pre-Workout-Runtime-Experience handoff functions that would populate them, e.g. `setPendingExecutableSession`, are called by nothing in production) — documented as intentionally-inert legacy route files rather than deleted, since they were never named in this sprint's cleanup list and their backing screens each have their own passing test suite
- **Legacy screen cleanup:** re-confirmed all six screens Sprint 37.1 flagged (`ProgressScreen.tsx`, `WorkoutScreen.tsx`, `DashboardScreen.tsx`, `NutritionScreen.tsx`, `CoachScreen.tsx`, `ProfileScreen.tsx`) still have zero production references — every real route already imports its `*-experience`/`home`/`workout-runtime` replacement directly — and deleted all six, plus the two dedicated test files that exclusively exercised a deleted screen (`ProfileScreen.test.tsx`, `CoachScreen.test.tsx`); their now-orphaned transitive dependencies (`features/progress`'s UI hook, `features/nutrition`'s legacy UI components, `features/coach`, `useCoachConversation`, `useCoachPrompt`, `features/profile`'s edit-form stack, `useCurrentUser`) were deliberately left in place, since each still has its own independent test suite and/or live sibling exports still used by production (`MockProgressService` behind `progress-experience`'s Mock provider, nutrition domain models behind `integrations/nutrition-progress`, `SettingsRow` behind `SettingsScreen`/`AppearanceScreen`/`ThemeScreen`) — a broader dead-code sweep is scoped as a future follow-up, not this sprint
- **Mock/demo audit:** searched the full authenticated route tree for `Mock*`/demo/sample/fake/placeholder patterns; zero production-reachable findings — every production screen mounts its hook/ViewModel with no `service` override and takes the hydrated-runtime path; `MockProgressAnalyticsService` (Sprint 35.3/37.1's documented sole working Progress Analytics implementation) and the Sprint 30.x Mock adapter family (`MockAuthenticationProvider`/`MockBackendProvider`/`MockLogger`) reconfirmed as intentional production infrastructure, unchanged
- **Runtime lifecycle / athlete isolation / persistence-restart / failure-recovery / write-through concurrency:** read-only re-verification against current code, with file/line evidence, of all nine audited areas — authenticated startup order (Bootstrap → Hydration → Dashboard Restore → Observer → ready), `retrySession()`'s unconditional full pipeline reset (Sprint 36.3), the logout reset cascade (Observer → Write-Through → Dashboard Restore → Hydration → Bootstrap-incl.-Composition-Root → Session, Sprint 33.8/36.1), hydration's athlete-scope filter (Sprint 36.1) and payload-internal ownership guard (Sprint 36.5) wired into every domain, all eight domains' (Identity/Workspace/Workout/Nutrition/Recovery/Goals/Coach/Notifications) SQLite round-trip, `triggerWriteThrough()`'s `.catch()` preventing unhandled rejections (Sprint 36.4), and the Sprint 36.5/36.6 write-through concurrency guards (pre-I/O stale-sequence rejection, post-I/O second sequence re-check, never-per-mutation-reset `sessionEpoch`, `stale_write_through_epoch`, `RuntimeObserver.start()`'s incremental partial-wrap cleanup) — all confirmed intact; zero runtime/persistence code changed, since none of the nine areas showed a concrete, provable defect
- **Performance audit:** catalogued every `setInterval`/`setTimeout`/`requestAnimationFrame` in `app/src`; confirmed the Sprint 36.6 rest-timer fix remains intact — `useRestTimer`'s two 1-second intervals call `WorkoutRuntimeViewModel.tickRestTimer()`/`tickDuration()`, which call only `notifyListeners()` (no write-through); no polling exists in `progress-experience`, `progress-analytics`, `coach-experience`, or `notification-center`; every timer/interval found has matching cleanup
- **Accessibility/UX regression audit (three narrow fixes, no visual change):** `AppButton` gained an additive, opt-in `selected?: boolean` prop (`undefined` by default — zero behavior change for its ~20 other call sites) that sets `accessibilityState.selected`; `TimeRangeSelector` now passes `selected={option.value === value}` so a screen reader can tell which time range is active (previously only conveyed by an untagged primary/secondary color variant); `NotificationCard`'s `accessibilityLabel` now includes `"{priority} priority."` when its Sprint 37.2 high-priority `Chip` is shown, instead of silently omitting it; `GoalsCard`/`NotificationPreferencesCard` (made pressable link-outs in Sprint 37.3) gained explicit `accessibilityLabel`s describing their navigation destination instead of relying on `AppCard`'s implicit child-text accessible name
...
- No runtime architecture redesign, no new runtime subsystem, no networking, no backend, no SQLite/schema change, no Admin changes, no live LLM provider, no repository contract change, no visual redesign; ADR-154; docs: PROJECT_STATE, CHANGELOG, DECISIONS

Previous: **37.1 — Production UI & Navigation Completion Audit** (2026-08-11)

- First Phase B (Product Completion) sprint after Phase A (Production Hardening, Sprints 36.1–36.6) concluded — audited whether every already-activated production experience (Home, Profile, Workout, Nutrition, Recovery, Goals, Coach, Notifications, Progress) is actually reachable through the production Expo Router tree and uses its correct runtime path; fixed only concrete, provable navigation defects, with an explicit "no redesign" mandate
- **Route tree audit:** confirmed the 6-tab bar (Home/Workout/Nutrition/Coach/Progress/Profile) + secondary `goals`/`recovery` + `settings` → `settings/appearance` → `settings/appearance/theme` + `workout/{analytics,complete,detail,history,records,session,summary}`; no duplicate route files
- **Gap 1 (Notifications unreachable):** `features/notification-center`'s `NotificationCenterScreen` was fully production-wired (hydrated Unified Workspace + Coach Timeline, Sprint 35.2) but had zero Expo Router route and no UI entry point — new `app/(app)/notifications/index.tsx` route + new bell-icon button on `HomeDashboardHeader` (rendered only when a handler is supplied) navigating to it
- **Gap 2 (Goals unreachable from Home):** Goals (activated Sprint 35.0, working `app/(app)/goals` route, already reachable from Coach's quick actions) had no Home quick action — new `VIEW_GOALS` quick action (`destination: "/(app)/goals"`, `enabled: workspace.goals.present`) added to production `mapWorkspaceToQuickActions.ts` and mirrored in test/preview `mapQuickActions.ts`
- **Gap 3 (Recovery routing bug):** both the Home "Recovery" quick action and Coach's "Recovery recommendation" card had `destination: "/(app)/(tabs)/progress"` — a real bug sending users to the Progress tab instead of Recovery; both corrected to `/(app)/recovery`
- **Gap 4 (dead-end buttons on primary screens):** Coach's header History/Settings buttons, Coach insight-card detail taps, Workout `ExerciseCard`'s "Details" button, and every Progress analytics card's `onPress` were always-visible, always-enabled controls navigating to a documented "navigation placeholder" destination with no matching route file — new pure `navigation/isReachableRoute.ts` allowlist utility lets the three screens compute each navigation callback through a local `reachableHandler` that disables the control (via the existing conditional-render pattern) instead of navigating to a not-found screen
- **Gap 5 (Profile Appearance orphaned):** `ProfileExperienceScreen` (Sprint 31.6) never carried forward the legacy `ProfileScreen`'s link to the already-working Settings → Appearance → Theme flow — `AppearanceCard` gained an optional `onPress` wired to `router.push("/(app)/settings/appearance")`
- **Mock production audit:** clean — no authenticated route or production hook defaults to a `Mock*Service`; `MockProgressAnalyticsService` preserved as the intentional sole working `ProgressAnalyticsService` per this sprint's explicit carve-out
- **Legacy path audit:** `ProgressScreen.tsx`, `WorkoutScreen.tsx`, `DashboardScreen.tsx`, `NutritionScreen.tsx`, `CoachScreen.tsx`, `ProfileScreen.tsx` confirmed unreachable (zero production references) — documented as safe-to-remove candidates for a future cleanup sprint, not deleted
- Verified with no code change: every screen's screen→hook→runtime path, loading/empty/error states, authentication boundaries, logout reset cascade, and restart/rehydration behavior
- Tests: new `navigation/__tests__/isReachableRoute.test.ts` (10 cases), new `coach-experience/__tests__/mapper.test.ts`, extended `dashboard-projection/__tests__/mapper.test.ts` and `home/__tests__/screen.test.tsx`; full suite (804 suites / 3342 tests) and typecheck remain green
- No runtime architecture redesign, no new runtime subsystem, no networking, no backend, no SQLite changes, no Admin changes, no live LLM provider, no visual redesign; ADR-151; docs: ARCHITECTURE, PROJECT_STATE, CHANGELOG, DECISIONS

Previous: **36.5 — Runtime Persistence Verification & Consistency Guard** (2026-08-11)
