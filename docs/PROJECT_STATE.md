...
Phase 31 — Product Development may begin. Immediate product candidate: wire Home UI providers (`features/home`) to `HomeExperienceService` dashboard APIs — presentation only, without embedding domain logic in UI. Replace Phase 30 mocks only behind existing contracts (new ADR per provider).
