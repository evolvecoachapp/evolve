...
- **Sprint 34.5:** Production Profile path reads hydrated `AthleteIdentityService` via `useProfile` + `applyHydratedProfile()`; Mock ProfileExperienceService removed from authenticated startup; profile update persistence not wired (follow-up)
