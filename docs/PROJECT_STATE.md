...
Current: **32.4 — Goal Progress → Progress Analytics Integration** (2026-08-10)

- Integration module (`integrations/goal-progress`) publishes immutable goal progress events and updates Progress Analytics read models through contracts
- Immutable models: `GoalProgressEvent`, `GoalProgressSnapshot`, `GoalMetric`, `GoalAnalyticsPayload`, `GoalProgressMetadata`, `GoalProgressResult`
- Supported events represent only: `GoalTrackingStarted`, `GoalProgressUpdated`, `GoalMilestoneReached`, `GoalTargetUpdated`, `GoalCompleted`, `GoalDeviationDetected`, `GoalAdherenceUpdated`, `GoalAchieved`
- `GoalProgressPublisher` publishes immutable events; `GoalProgressSubscriber` consumes via `ProgressAnalyticsService.applyGoalProgressEvent`
- Application APIs: `publishGoalProgress`, `publishGoalProgressUpdated`, `publishGoalMilestoneReached`, `publishGoalCompleted`
- Validation: missing event, duplicate event id, invalid payload, missing metadata, unsupported event type
- Composition Root registers `GoalProgressPublisher` + `GoalProgressSubscriber` via `GoalProgressIntegrationFactory`
- Goal Progress feature never depends on Progress Analytics internals; contract-only communication
- No analytics calculations, repository changes, persistence, networking, backend, synchronization, or event sourcing

Previous: **32.3 — Recovery → Progress Analytics Integration** (2026-08-10)
