...
const PREFIXES = Object.freeze(["adherence","compliance"] as string[]);
...
export function evaluateAdherence(input: GoalProgressInput): {
  ...
  const triggers = Object.freeze(
    keys.map((signalKey, i) =>
      freezeTrigger({
        id: `trigger:adherence:${i}:${input.id}`,
        kind: GoalAchievementKinds.ADHERENCE,
        signalKey,
  ...
  const candidates = Object.freeze(
    keys.length === 0
      ...
      : [
          freezeCandidate({
            id: `candidate:adherence:${input.id}`,
  ...
  const opportunities = Object.freeze(
    keys.length === 0
      ...
      : [
          freezeOpportunity({
            id: `opportunity:adherence:${input.id}`,
            category: GoalCategories.GENERAL,
            subjectId: input.athleteId,
...
export const AdherenceEvaluator = { detect: evaluateAdherence };
