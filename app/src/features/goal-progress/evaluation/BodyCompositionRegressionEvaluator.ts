...
const PREFIXES = Object.freeze(["regression","decline"] as string[]);
...
export function evaluateBodyCompositionRegression(input: GoalProgressInput): {
  ...
  const triggers = Object.freeze(
    keys.map((signalKey, i) =>
      freezeTrigger({
        id: `trigger:regression:${i}:${input.id}`,
        kind: GoalAchievementKinds.REGRESSION,
        signalKey,
  ...
  const candidates = Object.freeze(
    keys.length === 0
      ...
      : [
          freezeCandidate({
            id: `candidate:regression:${input.id}`,
  ...
  const opportunities = Object.freeze(
    keys.length === 0
      ...
      : [
          freezeOpportunity({
            id: `opportunity:regression:${input.id}`,
            category: GoalCategories.GENERAL,
            subjectId: input.athleteId,
...
export const BodyCompositionRegressionEvaluator = { detect: evaluateBodyCompositionRegression };
