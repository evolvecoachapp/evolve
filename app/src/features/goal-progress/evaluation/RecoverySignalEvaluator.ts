...
const PREFIXES = Object.freeze(["recovery"] as string[]);
...
export function evaluateRecoverySignal(input: GoalProgressInput): {
  ...
  const triggers = Object.freeze(
    keys.map((signalKey, i) =>
      freezeTrigger({
        id: `trigger:recovery:${i}:${input.id}`,
  ...
  const candidates = Object.freeze(
    keys.length === 0
      ...
      : [
          freezeCandidate({
            id: `candidate:recovery:${input.id}`,
  ...
  const opportunities = Object.freeze(
    keys.length === 0
      ...
      : [
          freezeOpportunity({
            id: `opportunity:recovery:${input.id}`,
            category: GoalCategories.GENERAL,
            subjectId: input.athleteId,
...
export const RecoverySignalEvaluator = { detect: evaluateRecoverySignal };
