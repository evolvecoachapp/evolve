...
const PREFIXES = Object.freeze(["progress","improve"] as string[]);
...
export function evaluateMilestone(input: GoalProgressInput): {
  ...
  const triggers = Object.freeze(
    keys.map((signalKey, i) =>
      freezeTrigger({
        id: `trigger:progress:${i}:${input.id}`,
        kind: GoalAchievementKinds.PROGRESS,
        signalKey,
  ...
  const candidates = Object.freeze(
    keys.length === 0
      ...
      : [
          freezeCandidate({
            id: `candidate:progress:${input.id}`,
  ...
  const opportunities = Object.freeze(
    keys.length === 0
      ...
      : [
          freezeOpportunity({
            id: `opportunity:progress:${input.id}`,
            category: GoalCategories.GENERAL,
            subjectId: input.athleteId,
...
export const MilestoneEvaluator = { detect: evaluateMilestone };
