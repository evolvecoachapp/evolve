...
const PREFIXES = Object.freeze(["plateau","flat"] as string[]);
...
export function evaluateStrengthPlateau(input: GoalProgressInput): {
  ...
  const triggers = Object.freeze(
    keys.map((signalKey, i) =>
      freezeTrigger({
        id: `trigger:plateau:${i}:${input.id}`,
        kind: GoalAchievementKinds.PLATEAU,
        signalKey,
  ...
  const opportunities = Object.freeze(
    keys.length === 0
      ...
      : [
          freezeOpportunity({
            id: `opportunity:plateau:${input.id}`,
            category: GoalCategories.GENERAL,
            subjectId: input.athleteId,
...
export const StrengthPlateauEvaluator = { detect: evaluateStrengthPlateau };
