...
export function CoachInsightsCard({ insights, onPress }: CoachInsightsCardProps) {
  const styles = useThemedStyles(({ colors, typography }) => ({ body: { gap: spacing.md }, title: { ...typography.title3 }, item: { gap: spacing.xs }, metric: { ...typography.caption, color: colors.pulse }, summary: { ...typography.callout, color: colors.inkMuted } }));
  return (
