import {
  createNutritionProgressResult,
  createNutritionProgressSnapshot,
  type NutritionProgressEvent,
  type NutritionProgressResult,
  type NutritionProgressSnapshot,
} from "../models";
import { validateNutritionProgressEvent } from "../validation";
import type {
  NutritionProgressEventSubscriber,
  NutritionProgressPublisher as NutritionProgressPublisherContract,
} from "./NutritionProgressPublisher";

export class DefaultNutritionProgressPublisher implements NutritionProgressPublisherContract {
  readonly id = "nutrition-progress-publisher";

  private readonly publishedEventIds = new Set<string>();
  private lastEvent: NutritionProgressEvent | null = null;

  constructor(
    private readonly subscribers: readonly NutritionProgressEventSubscriber[] = [],
  ) {}

  async publish(event: NutritionProgressEvent): Promise<NutritionProgressResult> {
    validateNutritionProgressEvent({
      event,
      publishedEventIds: [...this.publishedEventIds],
    });

    const frozenEvent = Object.freeze({
      ...event,
      metadata: Object.freeze({ ...event.metadata }),
      payload: Object.freeze({
        ...event.payload,
        metrics: Object.freeze([...event.payload.metrics]),
      }),
    });

    this.publishedEventIds.add(frozenEvent.id);
    this.lastEvent = frozenEvent;

    const subscriberResults = await Promise.all(
      this.subscribers.map((subscriber) => subscriber.onEvent(frozenEvent)),
    );

    return createNutritionProgressResult({
      eventId: frozenEvent.id,
      accepted: true,
      publishedAt: frozenEvent.metadata.publishedAt,
      subscriberResults: Object.freeze([...subscriberResults]),
    });
  }

  getPublishedEventIds(): readonly string[] {
    return Object.freeze([...this.publishedEventIds]);
  }

  getSnapshot(): NutritionProgressSnapshot {
    return createNutritionProgressSnapshot({
      publishedEventCount: this.publishedEventIds.size,
      lastEventId: this.lastEvent?.id ?? null,
      lastEventType: this.lastEvent?.type ?? null,
      capturedAt: this.lastEvent?.metadata.publishedAt ?? new Date(0).toISOString(),
    });
  }
}
